...
async function initializeModels(): Promise<void> {
  if (isTruthy(modelsInitialized)) return
...
export async function generateContentFingerprint(text: string, mediaUrls: readonly string[]): Promise<string> {
  const content = `${String(text.trim().toLowerCase() ?? '')}|${String([...mediaUrls].sort().join('|') ?? '')}`
  ...
  try {
    ...
    return `fp_${String(hashHex.substring(0, 16) ?? '')}`
  } catch (error) {
    ...
    return `fp_${String(Math.abs(hash).toString(36) ?? '')}`
  }
}
