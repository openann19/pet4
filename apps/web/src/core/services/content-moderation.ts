...
function isToxicityModel(
  value: unknown
): value is { classify: (texts: string[]) => Promise<ToxicityPrediction[]> } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'classify' in value &&
...
function isNSFWModel(
  value: unknown
): value is { classify: (img: HTMLImageElement) => Promise<NSFWPrediction[]> } {
  return (
    typeof value === 'object' &&
    value !== null &&
    'classify' in value &&
...
async function detectNSFWText(text: string): Promise<number> {
  ...
  try {
    ...
    if (toxicityModel && isToxicityModel(toxicityModel)) {
      ...
      const sexualExplicit = predictions.find(
        (p): p is ToxicityPrediction => isToxicityPrediction(p) && p.label === 'sexual_explicit'
      );
      if (sexualExplicit?.results[0]?.match) {
        return sexualExplicit.results[0].probabilities[1] ?? 0;
      }
    }

    // Fallback: keyword-based detection
    const nsfwKeywords = ['explicit', 'adult', 'nsfw', 'xxx', 'porn', 'sexual', 'nude', 'naked'];
...
export async function moderatePost(
  ...
  // Combine results
  const maxNSFWScore = Math.max(textModeration.nsfwScore, ...mediaResults.map((r) => r.nsfwScore));
  ...
  const requiresReview =
    textModeration.requiresReview || mediaResults.some((r) => r.requiresReview);

  const passed = textModeration.passed && mediaResults.every((r) => r.passed);
