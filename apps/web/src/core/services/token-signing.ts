...
function getConfigFromAdmin(): TokenSigningConfig | null {
  ...
  try {
    ...
    if (!adminConfig.livekit?.enabled || !adminConfig.livekit?.apiKey || !adminConfig.livekit?.apiSecret) {
      return null
    }
...
export async function signLiveKitToken(
  ...
  if (typeof window !== 'undefined') {
    logger.error('Client-side token signing is not secure. This should be implemented server-side.', {
      room: payload.room,
