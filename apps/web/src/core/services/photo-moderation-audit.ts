...
export class PhotoModerationAuditService {
  ...
  async logEvent(options: AuditLogOptions): Promise<PhotoModerationAuditLog> {
    try {
      const auditId = `audit_${String(Date.now() ?? '')}_${String(Math.random().toString(36).substr(2, 9) ?? '')}`
      ...
      const auditKey = `${String(AUDIT_PREFIX ?? '')}${String(auditId ?? '')}`
   ...
   */
  async getPhotoAuditLogs(
    photoId: string
  ): Promise<PhotoModerationAuditLog[]> {
    try {
      const indexKey = `${String(AUDIT_INDEX_PREFIX ?? '')}photo:${String(photoId ?? '')}`
  ...
  async getUserAuditLogs(
    ...
    try {
      const indexKey = `${String(AUDIT_INDEX_PREFIX ?? '')}user:${String(userId ?? '')}`
      const auditIds = await storage.get<string[]>(indexKey) ?? []
  ...
  async getActionAuditLogs(
    ...
    try {
      const indexKey = `${String(AUDIT_INDEX_PREFIX ?? '')}action:${String(action ?? '')}`
      const auditIds = await storage.get<string[]>(indexKey) ?? []
