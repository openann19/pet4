...
describe('Admin Integration Tests', () => {
  ...
  describe('Admin Sync Service', () => {
    ...
    it('should handle multiple listeners', async () => {
      adminSyncService.initialize(realtimeClient)
      const listener1: AdminSyncListener = { onAdminAction: vi.fn() as (event: AdminSyncEvent) => void }
      const listener2: AdminSyncListener = { onAdminAction: vi.fn() as (event: AdminSyncEvent) => void }
  ...
  describe('Error Handling', () => {
    it('should handle uninitialized service gracefully', async () => {
      ...
      await expect(
        adminSyncService.broadcastAdminAction(
          'test',
          'admin-123',
          'Admin',
          'test',
