...
import type { PhotoModerationStatus } from '@/core/domain/photo-moderation';
...
export class PhotoModerationQueueService {
   ...
   */
  async enqueue(photoId: string, priority = 0): Promise<void> {
    try {
      ...
      const queueKey = `${QUEUE_PREFIX}${photoId}`;
   ...
   */
  async updateStatus(photoId: string, status: PhotoModerationStatus): Promise<void> {
    try {
      const queueKey = `${QUEUE_PREFIX}${photoId}`;
      const item = await storage.get<QueueItem>(queueKey);

      if (!item) {
        throw new Error(`Queue item not found: ${photoId}`);
      }
   ...
   */
  async assignBatch(photoIds: string[], moderatorId: string): Promise<string> {
    try {
      ...
      const batchKey = `${BATCH_PREFIX}${batchId}`;
