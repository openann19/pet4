...
function adjustLightness(oklch: string, delta: number): string {
  ...
  if (!match?.[1] || !match[2] || !match[3]) return oklch;
 ...
 */
export function generateButtonTokensForTheme(preset: ThemePresetConfig): ButtonColorTokens {
  ...
  const secondaryFg = colors.secondaryForeground
    ? oklchToHex(colors.secondaryForeground)
    : '#FFFFFF';
  const accessibleSecondaryFg = getAccessibleTextColor(secondaryBg);
  const secondaryFgHex = meetsAAUI(accessibleSecondaryFg, secondaryBg)
    ? accessibleSecondaryFg
    : secondaryFg;
  ...
  const destructiveFg = colors.destructiveForeground
    ? oklchToHex(colors.destructiveForeground)
    : '#FFFFFF';
  const accessibleDestructiveFg = getAccessibleTextColor(destructiveBg);
  const destructiveFgHex = meetsAAUI(accessibleDestructiveFg, destructiveBg)
    ? accessibleDestructiveFg
    : destructiveFg;
