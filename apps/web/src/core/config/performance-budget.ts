...
import type { PerformanceBudgetConfig, BundleBudget, LoadTimeBudget, RuntimeBudget } from './performance-budget.config'
...
export class PerformanceBudget {
  ...
  checkBundles(metrics: BundleMetrics): BudgetViolation[] {
    ...
    if (metrics.initial > budget.initial) {
      violations.push({
        ...
        severity: 'error',
        message: `Initial bundle size ${String(metrics.initial ?? '')}KB exceeds budget of ${String(budget.initial ?? '')}KB`,
      })
    }
    ...
    if (metrics.total > budget.total) {
      violations.push({
        ...
        budget: budget.total,
        severity: 'error',
        message: `Total bundle size ${String(metrics.total ?? '')}KB exceeds budget of ${String(budget.total ?? '')}KB`,
      })
    }
    ...
    for (const [chunkName, size] of Object.entries(metrics.chunks)) {
      if (size > budget.chunk) {
        violations.push({
          ...
          budget: budget.chunk,
          severity: 'warning',
          message: `Chunk "${String(chunkName ?? '')}" size ${String(size ?? '')}KB exceeds budget of ${String(budget.chunk ?? '')}KB`,
        })
      }
    }
    ...
    for (const [vendorName, size] of Object.entries(metrics.vendors)) {
      if (size > budget.vendor) {
        violations.push({
          ...
          severity: 'warning',
          message: `Vendor chunk "${String(vendorName ?? '')}" size ${String(size ?? '')}KB exceeds budget of ${String(budget.vendor ?? '')}KB`,
        })
      }
    }
  ...
  checkLoadTimes(metrics: LoadTimeMetrics): BudgetViolation[] {
    ...
    if (metrics.fcp !== undefined && metrics.fcp > budget.fcp) {
      violations.push({
        ...
        budget: budget.fcp,
        severity: 'error',
        message: `First Contentful Paint ${String(metrics.fcp ?? '')}ms exceeds budget of ${String(budget.fcp ?? '')}ms`,
      })
    }
    ...
    if (metrics.lcp !== undefined && metrics.lcp > budget.lcp) {
      violations.push({
        ...
        budget: budget.lcp,
        severity: 'error',
        message: `Largest Contentful Paint ${String(metrics.lcp ?? '')}ms exceeds budget of ${String(budget.lcp ?? '')}ms`,
      })
    }
    ...
    if (metrics.tti !== undefined && metrics.tti > budget.tti) {
      violations.push({
        ...
        budget: budget.tti,
        severity: 'error',
        message: `Time to Interactive ${String(metrics.tti ?? '')}ms exceeds budget of ${String(budget.tti ?? '')}ms`,
      })
    }
    ...
    if (metrics.fid !== undefined && metrics.fid > budget.fid) {
      violations.push({
        ...
        budget: budget.fid,
        severity: 'warning',
        message: `First Input Delay ${String(metrics.fid ?? '')}ms exceeds budget of ${String(budget.fid ?? '')}ms`,
      })
    }
    ...
    if (metrics.cls !== undefined && metrics.cls > budget.cls) {
      violations.push({
        ...
        budget: budget.cls,
        severity: 'warning',
        message: `Cumulative Layout Shift ${String(metrics.cls ?? '')} exceeds budget of ${String(budget.cls ?? '')}`,
      })
    }
  ...
  checkRuntime(metrics: RuntimeMetrics): BudgetViolation[] {
    ...
    if (metrics.fps !== undefined && metrics.fps < budget.fps) {
      violations.push({
        ...
        budget: budget.fps,
        severity: 'warning',
        message: `FPS ${String(metrics.fps ?? '')} is below target of ${String(budget.fps ?? '')}`,
      })
    }
    ...
    if (metrics.frameTime !== undefined && metrics.frameTime > budget.frameTime) {
      violations.push({
        ...
        severity: 'warning',
        message: `Frame time ${String(metrics.frameTime ?? '')}ms exceeds budget of ${String(budget.frameTime ?? '')}ms`,
      })
    }
    ...
    if (metrics.memory !== undefined && metrics.memory > budget.memory) {
      violations.push({
        ...
        severity: 'warning',
        message: `Memory usage ${String(metrics.memory ?? '')}MB exceeds budget of ${String(budget.memory ?? '')}MB`,
      })
    }
    ...
    if (metrics.jsExecutionTime !== undefined && metrics.jsExecutionTime > budget.jsExecutionTime) {
      violations.push({
        ...
        severity: 'warning',
        message: `JS execution time ${String(metrics.jsExecutionTime ?? '')}ms exceeds budget of ${String(budget.jsExecutionTime ?? '')}ms`,
      })
    }
  ...
  generateReport(): string {
    ...
    if (errors.length > 0) {
      report += `❌ Errors (${String(errors.length ?? '')}):\n`
      errors.forEach((violation) => {
        report += `   • ${String(violation.message ?? '')}\n`
      })
      report += '\n'
    }

    if (warnings.length > 0) {
      report += `⚠️  Warnings (${String(warnings.length ?? '')}):\n`
  ...
  logViolations(): void {
    ...
    if (errors.length > 0) {
      logger.error(`Performance budget errors: ${String(errors.length ?? '')}`)
      errors.forEach((violation) => {
    ...
    if (warnings.length > 0) {
      logger.warn(`Performance budget warnings: ${String(warnings.length ?? '')}`)
      warnings.forEach((violation) => {
