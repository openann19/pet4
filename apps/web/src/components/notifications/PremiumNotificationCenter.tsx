  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:max-w-2xl p-0 overflow-hidden flex flex-col">
              ...
              <div>
                ...
                {notifications.unreadCount > 0 && (
                  <p className="text-sm text-muted-foreground mt-0.5">
                    {notifications.unreadCount} unread {archivedCount > 0 && `· ${String(archivedCount ?? '')} archived`}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem 
                    onClick={() => { filtersWithNotifications.setView(filtersWithNotifications.view === 'grouped' ? 'list' : 'grouped'); }}
                  >
        ...
        <Tabs value={filtersWithNotifications.filter} onValueChange={(value) => { filtersWithNotifications.setFilter(value as NotificationFilter); }} className="flex-1 flex flex-col overflow-hidden">
          ...
          <TabsContent value={filtersWithNotifications.filter} className="flex-1 overflow-hidden m-0">
            <ScrollArea className="h-full">
              <div className="px-6 py-4">
                {filtersWithNotifications.view === 'grouped' && notifications.groupedNotifications.length > 0 ? (
