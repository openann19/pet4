...
export function PremiumNotificationBell(): JSX.Element {
  ...
  useEffect(() => {
    try {
      ...
      if (newNotifs.length > 0 && !isOpen) {
        ...
        if (isTruthy(notificationTimerRef.current)) {
  ...
  const ariaLabel = useMemo<string>(() => {
    ...
    if (unreadCount > 0) {
      parts.push(`${String(unreadCount ?? '')} unread`)
    }
    if (urgentCount > 0) {
      parts.push(`${String(urgentCount ?? '')} urgent`)
    }
    return parts.join(' - ')
...
function BellIcon({ hasNewNotification, unreadCount, hasUrgent }: BellIconProps): JSX.Element {
  ...
  const iconClassName = useMemo<string>(() => {
    if (isTruthy(hasUrgent)) return 'text-destructive'
    ...
    >
      <AnimatedView style={pulseStyle}>
        <Badge 
          ...
          aria-label={`${String(unreadCount ?? '')} unread notification${String(unreadCount !== 1 ? 's' : '' ?? '')}`}
        >
...
function UrgentGlow(): JSX.Element {
  ...
  const glowStyle = useAnimatedStyle(() => {
    ...
    return {
      boxShadow: `0 0 ${String(radius ?? '')}px rgba(239, 68, 68, ${String(opacity ?? '')})`
    }
  }) as AnimatedStyle
