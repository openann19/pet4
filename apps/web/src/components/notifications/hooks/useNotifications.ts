...
import type { PremiumNotification, NotificationGroup, NotificationFilter, NotificationPreferences } from '../types'
...
export function useNotifications(options: UseNotificationsOptions): UseNotificationsReturn {
  ...
  const [notifications, setNotifications] = useLocalStorage<PremiumNotification[]>('premium-notifications', [])

  const allNotifications = useMemo(
    () => (notifications ?? []) as PremiumNotification[],
    [notifications]
  )
  ...
  const groupedNotifications = useMemo(() => {
    ...
    filteredNotifications.forEach((notification: PremiumNotification) => {
      const groupKey = notification.groupId ?? `${String(notification.type ?? '')}-${notification.metadata?.userName ?? notification.metadata?.petName ?? 'solo'}`
    ...
    const notificationGroups: NotificationGroup[] = Array.from(groups.entries()).map(([id, notifs]) => {
      ...
      return {
        ...
        summary: notifs.length > 1 
          ? `${String(notifs.length ?? '')} notifications` 
          : latest.message,
  ...
  const archiveGroup = useCallback((groupId: string) => {
    setNotifications((prev: PremiumNotification[]) => 
      (prev ?? []).map((n: PremiumNotification) => {
        const groupKey = n.groupId ?? `${String(n.type ?? '')}-${n.metadata?.userName ?? n.metadata?.petName ?? 'solo'}`
