...
function formatBadge(count: number | undefined, locale: Locale) {
  ...
  return { display: new Intl.NumberFormat(locale).format(count), title: new Intl.NumberFormat(locale).format(count) };
}
...
export function NotificationTabs({
  ...
  const segments = useMemo(() => (providedSegments && providedSegments.length ? providedSegments : DEFAULT_SEGMENTS), [providedSegments]);

  const [counts, setCounts] = useState<UnreadCounts>(() => unread ?? ({} as UnreadCounts));
  useEffect(() => { if (isTruthy(unread)) setCounts(unread); }, [unread]);
  ...
  const [focusIndex, setFocusIndex] = useState(() => Math.max(0, segments.findIndex(s => s.key === selected)));
  useEffect(() => { setFocusIndex(Math.max(0, segments.findIndex(s => s.key === selected))); }, [segments, selected]);
  ...
  const activate = useCallback((key: TabKey) => {
    ...
    setAnnounceMsg(locale === "bg" ? `Преминахте към ${String(label ?? '')}` : `Switched to ${String(label ?? '')}`);
  ...
  const updateIndicator = useCallback(() => {
    ...
    const active = container.querySelector<HTMLButtonElement>(`button[role="tab"][data-key="${String(selected ?? '')}"]`);
    if (!active) return;
  ...
  const onKeyDownTab = (e: React.KeyboardEvent, index: number) => {
    ...
    if (e.key === "ArrowRight") { e.preventDefault(); setFocusIndex(index === last ? 0 : index + 1); }
    ...
    <div ref={shellRef} className={cx("w-full flex flex-col", className)} data-testid="notification-tabs">
      ...
      <div className="relative">
        ...
        <div
          ref={tablistRef}
          role="tablist"
          aria-label={ariaLabel ?? (locale === "bg" ? "Сегменти известия" : "Notification segments")}
          className={cx(
            ...
            return (
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <span className="max-w-[10ch] truncate" title={label}>{label}</span>
                  {badgeData && (
                    <span
                      className={cx(
                      ...
                      aria-label={locale === "bg" ? `непрочетени ${String(badgeData.title ?? '')}` : `unread ${String(badgeData.title ?? '')}`}
                    >
                      {badgeData.display}
                    </span>
                  )}
