...
import { useHoverLift, type UseHoverLiftReturn } from '@/effects/reanimated/use-hover-lift'
...
export default function PetDetailDialog({ pet, open, onOpenChange }: PetDetailDialogProps) {
  ...
  const closeButtonHover: UseHoverLiftReturn = useHoverLift()
  const closeButtonTap: UseBounceOnTapReturn = useBounceOnTap()
        ...
        >
          ...
          <div className="relative h-[400px] bg-linear-to-br from-muted/50 to-muted overflow-hidden group">
            ...
            >
              <img
                src={currentPhoto}
                alt={`${String(pet.name ?? '')} photo ${String(currentIndex + 1 ?? '')}`}
