...
describe('StoryFilterSelector', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should render all category buttons', () => {
      ...
      FILTER_CATEGORIES.forEach((category) => {
        const button = screen.getByLabelText(`Filter category: ${String(category.name ?? '')}`)
    ...
    it('should render filter grid', () => {
      ...
      STORY_FILTERS.forEach((filter) => {
        const filterButton = screen.getByLabelText(`Select filter: ${String(filter.name ?? '')}`)
    ...
    it('should mark selected filter as pressed', () => {
      ...
      const selectedButton = screen.getByLabelText(`Select filter: ${String(mockFilter.name ?? '')}`)
  ...
  describe('Search Functionality', () => {
    it('should filter filters by search query', async () => {
      ...
      await waitFor(() => {
        ...
        if (isTruthy(warmFilter)) {
          expect(screen.getByLabelText(`Select filter: ${String(warmFilter.name ?? '')}`)).toBeInTheDocument()
        }
      })
    })
    ...
    it('should clear filters when search is cleared', async () => {
      ...
      await waitFor(() => {
        STORY_FILTERS.forEach((filter) => {
          expect(screen.getByLabelText(`Select filter: ${String(filter.name ?? '')}`)).toBeInTheDocument()
        })
      })
    })
  })
  ...
  describe('Category Selection', () => {
    it('should filter by selected category', async () => {
      ...
      if (isTruthy(naturalCategory)) {
        const categoryButton = screen.getByLabelText(`Filter category: ${String(naturalCategory.name ?? '')}`)
        await act(async () => {
    ...
    it('should call onSelectFilter when filter is clicked', async () => {
      ...
      if (isTruthy(firstFilter)) {
        const filterButton = screen.getByLabelText(`Select filter: ${String(firstFilter.name ?? '')}`)
        await act(async () => {
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels for all interactive elements', () => {
      ...
      FILTER_CATEGORIES.forEach((category) => {
        expect(screen.getByLabelText(`Filter category: ${String(category.name ?? '')}`)).toBeInTheDocument()
      })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle rapid category changes', async () => {
      ...
      await waitFor(() => {
        ...
        if (isTruthy(lastCategory)) {
          const lastButton = screen.getByLabelText(`Filter category: ${String(lastCategory.name ?? '')}`)
