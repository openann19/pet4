...
const ALL_PERSONALITIES = [
  'playful', 'calm', 'energetic', 'gentle', 'social', 'independent',
  'affectionate', 'curious', 'protective', 'loyal', 'friendly', 'outgoing',
  'confident', 'quiet', 'relaxed', 'active', 'adventurous', 'cuddly', 'loving', 'intelligent'
]

const ALL_INTERESTS = [
  'fetch', 'swimming', 'hiking', 'cuddling', 'playing', 'training',
  'running', 'walking', 'agility', 'tricks', 'sleeping', 'eating',
  'exploring', 'meeting new friends', 'car rides', 'beach', 'park', 'toys'
]

const ALL_LOOKING_FOR = [
  'playdate', 'walking buddy', 'training partner', 'best friend',
  'adventure companion', 'cuddle buddy', 'active friend', 'gentle friend'
]

export default function DiscoveryFilters() {
  const [preferences, setPreferences] = useStorage<DiscoveryPreferences>('discovery-preferences', DEFAULT_PREFERENCES)
  ...
  const [photoQuality, setPhotoQuality] = useState(currentPrefs.mediaFilters?.photoQuality || 'any')
  ...
  const [activeToday, setActiveToday] = useState(currentPrefs.advancedFilters?.activeToday || false)
  const [hasStories, setHasStories] = useState(currentPrefs.advancedFilters?.hasStories || false)
  const [respondQuickly, setRespondQuickly] = useState(currentPrefs.advancedFilters?.respondQuickly || false)
  const [superLikesOnly, setSuperLikesOnly] = useState(currentPrefs.advancedFilters?.superLikesOnly || false)
          ...
          <TabsContent value="basic" className="flex-1 overflow-hidden mt-4">
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-medium">{minAge} {minAge === 0 ? 'year' : 'years'}</span>
                  </div>
                  <Slider
                    value={[minAge]}
            ...
            <div>
              <Label className="text-base mb-3 block font-semibold">
                Personality Traits {personalities.length > 0 && <span className="text-xs text-muted-foreground">({personalities.length} selected)</span>}
              </Label>
              <p className="text-xs text-muted-foreground mb-3">Find pets with these personality traits</p>
              <div className="flex flex-wrap gap-2">
            ...
            <div>
              <Label className="text-base mb-3 block font-semibold">
                Interests {interests.length > 0 && <span className="text-xs text-muted-foreground">({interests.length} selected)</span>}
              </Label>
              <p className="text-xs text-muted-foreground mb-3">Find pets who enjoy these activities</p>
              <div className="flex flex-wrap gap-2">
            ...
            <div>
              <Label className="text-base mb-3 block font-semibold">
                Looking For {lookingFor.length > 0 && <span className="text-xs text-muted-foreground">({lookingFor.length} selected)</span>}
              </Label>
              <p className="text-xs text-muted-foreground mb-3">Find pets seeking these connections</p>
              <div className="flex flex-wrap gap-2">
            ...
            <div>
              <Label className="text-base mb-4 block font-semibold">Minimum Compatibility Score</Label>
              <p className="text-xs text-muted-foreground mb-3">Only show pets with at least this compatibility</p>
              <div className="flex items-center justify-between mb-2">
              ...
              <ScrollArea className="h-full -mx-6 px-6">
                <div className="space-y-6 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-3">Filter profiles by photo aspect ratio</p>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-3">Prefer high-quality or verified photos</p>
                    <div className="flex flex-col gap-2">
                      {[
                        ...
                        <MotionView as="button"
                          ...
                          className={`p-4 rounded-lg border-2 transition-all text-left ${
                            String(photoQuality === value
                                                            ? 'border-primary bg-primary/10'
                                                            : 'border-border hover:border-primary/50' ?? '')
                          }`}
                        >
                          <div className="font-medium text-sm flex items-center gap-2">
                            {photoQuality === value && <CheckCircle size={16} weight="fill" className="text-primary" />}
                            {label}
                          </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg border bg-card">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground mt-1">Only show profiles with video content</div>
                      </div>
                      <Switch
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-3">Profiles must have at least this many photos</p>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-muted-foreground">Minimum</span>
                      <span className="text-sm font-medium">{minPhotos} photo{minPhotos !== 1 ? 's' : ''}</span>
                    </div>
                    <Slider
                      value={[minPhotos]}
              ...
              <ScrollArea className="h-full -mx-6 px-6">
                <div className="space-y-4 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-4">Find the most active and responsive matches</p>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">ID or photo verified accounts</div>
                          </div>
                        </div>
                        <Switch
                          checked={verified}
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">Online in the last 24 hours</div>
                          </div>
                        </div>
                        <Switch
                          checked={activeToday}
                          onCheckedChange={setActiveToday}
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">Posted stories recently</div>
                          </div>
                        </div>
                        <Switch
                          checked={hasStories}
                          onCheckedChange={setHasStories}
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">Average response under 1 hour</div>
                          </div>
                        </div>
                        <Switch
                          checked={respondQuickly}
                          onCheckedChange={setRespondQuickly}
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-lg border bg-card hover:bg-accent/5 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground mt-0.5">Show only profiles that super liked you</div>
                          </div>
                        </div>
                        <Switch
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Advanced filters help you find the most compatible and active matches. Some filters may reduce the number of available profiles.
                        </div>
                      </div>
                    </div>
                  </div>
