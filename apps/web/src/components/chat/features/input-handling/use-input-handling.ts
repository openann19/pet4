...
export function useInputHandling(
  options: UseInputHandlingOptions
): UseInputHandlingReturn {
  ...
  const handleSuggestionSelect = useCallback((suggestion: SmartSuggestion): void => {
  ...
  const handleStickerSelect = useCallback((emoji: string): void => {
