...
export default function AdvancedChatWindow({
  ...
  const [messages, setMessages] = useStorage<ChatMessage[]>(`chat-messages-${String(room.id ?? '')}`, [])
  const [inputValue, setInputValue] = useState('')
  const [showStickers, setShowStickers] = useState(false)
  const [showTemplates, setShowTemplates] = useState(false)
  const [isRecordingVoice, setIsRecordingVoice] = useState(false)
  const [awayMode, setAwayMode] = useStorage<boolean>(`away-mode-${String(currentUserId ?? '')}`, false)
  ...
  const onTranslate = async (messageId: string): Promise<void> => {
    ...
    try {
      const prompt = buildLLMPrompt`Translate to English, return text only: "${String(m.content ?? '')}"`
