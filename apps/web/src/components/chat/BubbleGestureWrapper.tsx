...
export interface BubbleGestureWrapperProps {
  ...
  onPanUpdate?: (event: { translationX: number; translationY: number; x: number; y: number }) => void
  onPanEnd?: (event: { translationX: number; translationY: number; velocityX: number; velocityY: number }) => void
...
export function BubbleGestureWrapper({
  ...
  const handleMouseUp = useCallback((e: MouseEvent) => {
    ...
    if (wasPressed && panStartRef.current) {
      ...
      if (duration < LONG_PRESS_DURATION && Math.abs(translationX) < 10 && Math.abs(translationY) < 10) {
        if (isTruthy(onPress)) {
          onPress({ x: e.clientX, y: e.clientY })
        }
      } else if (isTruthy(onPanEnd)) {
        onPanEnd({
          translationX,
          translationY,
