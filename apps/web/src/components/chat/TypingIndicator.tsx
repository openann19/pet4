...
function TypingDot({ index }: TypingDotProps): JSX.Element {
  ...
  return (
    <AnimatedView style={animatedStyle} className="w-1 h-1 bg-primary rounded-full" />
  )
}
...
export default function TypingIndicator({ users }: TypingIndicatorProps): JSX.Element | null {
  ...
  return (
    <AnimatedView style={containerStyle} className="flex items-center gap-2" role="status" aria-live="polite">
