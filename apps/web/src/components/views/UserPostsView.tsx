...
export default function UserPostsView({
  ...
  useEffect(() => {
    ...
    if (isTruthy(observerTarget.current)) {
  ...
  const loadPosts = async (loadMore = false) => {
    if (isTruthy(loadingRef.current)) return
    ...
    try {
      ...
      if (response.posts.length > 0 && !userName) {
        ...
        if (isTruthy(firstPost)) {
          if (isTruthy(firstPost.authorName)) {
            setAuthorName(firstPost.authorName)
          }
          if (isTruthy(firstPost.authorAvatar)) {
