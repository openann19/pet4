...
export default function CommunityView(): JSX.Element {
  ...
  const loadAdoptionProfiles = useCallback(async (loadMore = false) => {
    if (isTruthy(adoptionLoadingRef.current)) return
  ...
  useEffect(() => {
    ...
    if (isTruthy(adoptionObserverTarget.current)) {
  ...
  const loadingSpinnerStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(loadingRotation.value ?? '')}deg` }],
  })) as AnimatedStyle
  
  const emptyStateStyle = useAnimatedStyle(() => ({
    transform: [
      { scale: emptyScale.value },
      { rotate: `${String(emptyRotation.value ?? '')}deg` }
    ]
  })) as AnimatedStyle
  ...
  const adoptionLoadingSpinnerStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(adoptionLoadingRotation.value ?? '')}deg` }],
  })) as AnimatedStyle
      ...
      >
        ...
        >
          <ArrowsClockwise 
            size={24} 
            weight="bold" 
            className={`${String(pullToRefresh.isRefreshing ? 'animate-spin' : '' ?? '')} text-primary`}
          />
        </AnimatedView>
      </AnimatedView>
