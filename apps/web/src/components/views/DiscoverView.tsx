...
export default function DiscoverView() {
      ...
      if (action === 'like') {
        setMatches((currentMatches) => {
          ...
          toast.success(t.common.itsAMatch, {
            description: `${String(userName ?? '')} ${String(t.common.and ?? '')} ${String(petName ?? '')} ${String(t.common.areNowConnected ?? '')}`,
          })
  ...
  const emptyStateIconStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotate: `${String(emptyStateIconRotate.value ?? '')}deg` }
    ]
  })) as AnimatedStyle

  const emptyStateRotateStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(emptyStateIconRotation.value ?? '')}deg` }]
  })) as AnimatedStyle
  ...
  const passButtonIconStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(passButtonIconRotate.value ?? '')}deg` }]
  })) as AnimatedStyle
  ...
  const compatibilityBadgeStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotate: `${String(compatibilityBadgeRotate.value ?? '')}deg` },
  ...
  const likeButtonShimmerStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: `${String(likeButtonShimmerX.value ?? '')}%` }]
  })) as AnimatedStyle
  ...
  useEffect(() => {
    if (isTruthy(breakdownDialog.isOpen)) {
