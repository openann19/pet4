...
vi.mock('@/components/adoption/AdoptionCard', () => ({
  default: ({ profile }: { profile: { _id: string } }) => <div data-testid={`adoption-card-${String(profile._id ?? '')}`}>Adoption Card</div>
}))
...
vi.mock('@/components/community/PostCard', () => ({
  default: ({ post }: { post: { id: string } }) => <div data-testid={`post-card-${String(post.id ?? '')}`}>Post Card</div>
}))
