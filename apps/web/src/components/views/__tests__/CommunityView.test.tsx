...
vi.mock('@/api/adoption-api', () => ({
  adoptionAPI: {
    getAdoptionProfiles: vi.fn(() =>
      Promise.resolve({ profiles: [], hasMore: false, nextCursor: undefined })
    ),
  },
}));
...
vi.mock('@/components/adoption/AdoptionCard', () => ({
  default: ({ profile }: { profile: { _id: string } }) => (
    <div data-testid={`adoption-card-${profile._id}`}>Adoption Card</div>
  ),
}));
...
vi.mock('@/components/community/PostCard', () => ({
  default: ({ post }: { post: { id: string } }) => (
    <div data-testid={`post-card-${post.id}`}>Post Card</div>
  ),
}));
