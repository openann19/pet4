...
describe('PetHealthDashboard', () => {
  ...
  describe('Vaccinations Tab', () => {
    it('should render vaccination records when they exist', async () => {
      mockStorageValues[`vaccinations-${String(mockPet.id ?? '')}`] = [mockVaccination]
    ...
    it('should display next due date for vaccination when available', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(new RegExp(`Next: ${String(nextDueText ?? '')}`))).toBeInTheDocument()
      })
    })
  ...
  describe('Health Records Tab', () => {
    it('should render health records when they exist', async () => {
      const user = userEvent.setup()
      mockStorageValues[`vaccinations-${String(mockPet.id ?? '')}`] = []
      mockStorageValues[`health-records-${String(mockPet.id ?? '')}`] = [mockHealthRecord]
      mockStorageValues[`reminders-${String(mockPet.id ?? '')}`] = []
  ...
  describe('Reminders Tab', () => {
    it('should render reminders when they exist', async () => {
      const user = userEvent.setup()
      mockStorageValues[`vaccinations-${String(mockPet.id ?? '')}`] = []
      mockStorageValues[`health-records-${String(mockPet.id ?? '')}`] = []
      mockStorageValues[`reminders-${String(mockPet.id ?? '')}`] = [mockReminder]
    ...
    it('should display overdue status for overdue reminders', async () => {
      ...
      mockStorageValues[`vaccinations-${String(mockPet.id ?? '')}`] = []
      mockStorageValues[`health-records-${String(mockPet.id ?? '')}`] = []
      mockStorageValues[`reminders-${String(mockPet.id ?? '')}`] = [overdueReminder]
