...
export function PetHealthDashboard({ pet, onClose }: PetHealthDashboardProps): JSX.Element {
  ...
  const [reminders, setReminders] = useStorage<VetReminder[]>(`reminders-${String(pet.id ?? '')}`, [])
        ...
        {healthSummary && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <Card>
              ...
              <CardContent>
                <Badge className={`${String(getStatusColor(healthSummary.vaccinationStatus) ?? '')} flex items-center gap-1 w-fit`}>
