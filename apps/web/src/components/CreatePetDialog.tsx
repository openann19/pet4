...
const PET_TYPES = [
  ...
  { value: 'cat' as PetType, label: 'Cat', icon: Cat, emoji: '🐈' },
...
export default function CreatePetDialog({ open, onOpenChange, editingPet }: CreatePetDialogProps) {
  ...
  const emojiStyle = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(emojiRotation.value ?? '')}deg` }]
  })) as AnimatedStyle
  ...
  const applyTemplate = (template: PetProfileTemplate) => {
    ...
    if (isTruthy(template.defaults.size)) setSize(template.defaults.size)
    if (isTruthy(template.defaults.breed)) setBreed(template.defaults.breed)
    if (isTruthy(template.defaults.bio)) setBio(template.defaults.bio)
  ...
  const handleSubmit = () => {
    ...
    if (isTruthy(editingPet)) {
      ...
      toast.success('Pet profile updated!', {
        description: `${String(name ?? '')}'s profile has been updated successfully.`
      })
    } else {
      setUserPets((current) => [...(current || []), petData])
      setAllPets((current) => [...(current || []), petData])
      toast.success('Pet profile created! 🎉', {
        description: `${String(name ?? '')} is ready to find perfect companions!`
      })
    }
              ...
              {templates.map((template) => (
                <AnimatedView
                  ...
                  className={`relative p-4 rounded-xl border-2 text-left transition-all duration-300 ${
                    String(selectedTemplate?.id === template.id
                                            ? 'border-primary bg-primary/10 shadow-lg shadow-primary/20'
                                            : 'border-border bg-card hover:border-primary/50 hover:bg-card/80' ?? '')
                  }`}
                >
