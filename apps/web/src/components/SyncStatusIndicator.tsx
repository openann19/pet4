...
export function SyncStatusIndicator() {
  ...
  const getIcon = () => {
    ...
    if (isTruthy(syncStatus.isSyncing)) {
  ...
  const getLabel = () => {
    ...
    if (syncStatus.failedActions > 0) {
      return `${String(syncStatus.failedActions ?? '')} failed`
    }
    if (syncStatus.pendingActions > 0) {
      return `${String(syncStatus.pendingActions ?? '')} pending`
    }
    return 'Synced'
  }
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className={`font-medium ${String(syncStatus.isOnline ? 'text-green-600' : 'text-destructive' ?? '')}`}>
