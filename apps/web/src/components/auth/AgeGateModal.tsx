...
export default function AgeGateModal({ open, onVerified, onClose }: AgeGateModalProps) {
  ...
  const handleVerify = async () => {
    ...
    if (age < MIN_AGE) {
      setError(
        t.auth?.ageTooYoung || `You must be at least ${String(MIN_AGE ?? '')} years old to use this app`
      )
      haptics.trigger('error')
      return
    }
    ...
    try {
      ...
      logger.error('Age verification error', error instanceof Error ? error : new Error(String(error)))
      ...
      >
        ...
        >
          ...
          <p className="text-muted-foreground mb-6">
            {t.auth?.ageVerificationDesc || `You must be at least ${String(MIN_AGE ?? '')} years old to use PawfectMatch.`}
          </p>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="country">
                {t.auth?.country || 'Country (Optional)'}
              </Label>
              <Input
                id="country"
                type="text"
