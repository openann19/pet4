...
describe('AdminLayout', () => {
  ...
  it('calls onExit when exit button is clicked', async () => {
    const user = userEvent.setup();
    render(
      <AdminLayout currentView="dashboard" onViewChange={mockOnViewChange} onExit={mockOnExit}>
        {mockChildren}
      </AdminLayout>
    );
  ...
  it('handles error in exit gracefully', async () => {
    ...
    render(
      <AdminLayout currentView="dashboard" onViewChange={mockOnViewChange} onExit={errorOnExit}>
        {mockChildren}
      </AdminLayout>
    );
  ...
  it('shows tooltips when sidebar is collapsed', () => {
    ...
    const buttonsWithTitle = menuButtons.filter((button) => button.getAttribute('title') !== null);
