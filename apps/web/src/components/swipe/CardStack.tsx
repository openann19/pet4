...
export function CardStack<T extends CardData>({
  ...
  const swipeHook = useNativeSwipe({
    ...
    hapticFeedback,
    reduceMotion,
    onCommit: useCallback((direction: 'left' | 'right') => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < Math.min(prefetchCount, availableCards.length - 1); i++) {
      ...
      if (isTruthy(card?.photo)) {
        prefetchUrls.push(card.photo)
      }
    }

    if (prefetchUrls.length > 0) {
      imagePrefetcher.prefetchBatch(prefetchUrls, { priority: 'high' })
        .catch((error: unknown) => {
          logger.warn('Image prefetch failed', error instanceof Error ? error : new Error(String(error)))
        })
    }
  ...
  if (availableCards.length === 0) {
    return <div className="flex items-center justify-center" style={{ width: cardWidth, height: cardHeight }} />
  }
        ...
        return (
          ...
          >
            {isTop ? (
              ...
              <div
                style={{
                  transform: `scale(${String(pooled.scale ?? '')}) translateY(${String(pooled.translateY ?? '')}px)`,
    ...
    <div className="relative" style={{ width, height }}>
      ...
      >
        <div className="absolute inset-0 pointer-events-none">
          <AnimatedView style={swipeHook.badgeStyle} className="absolute -top-12 left-1/2 -translate-x-1/2 z-50">
            <div
