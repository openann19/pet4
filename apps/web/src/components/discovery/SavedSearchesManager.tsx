        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="text-xs text-muted-foreground">
              {search.useCount > 0 && `Used ${String(search.useCount ?? '')} time${String(search.useCount !== 1 ? 's' : '' ?? '')}`}
              {search.lastUsed &&
                ` • Last: ${String(new Date(search.lastUsed).toLocaleDateString() ?? '')}`}
            </div>
...
export default function SavedSearchesManager({
  ...
  const handleSaveCurrentSearch = useCallback((): void => {
    try {
      ...
      toast.success('Search saved!', { description: `"${String(searchName ?? '')}" has been saved` })
  ...
  const handleApplySearch = useCallback((search: SavedSearch): void => {
    try {
      ...
      toast.success('Search applied', { description: `Filters updated to "${String(search.name ?? '')}"` })
  ...
  const handleDeleteSearch = useCallback((id: string, name: string): void => {
    try {
      ...
      triggerHaptic('light')
      toast.info('Search deleted', { description: `"${String(name ?? '')}" has been removed` })
  ...
  const getPreferencesSummary = useCallback((prefs: DiscoveryPreferences): string => {
    ...
    if (prefs.minAge !== 0 || prefs.maxAge !== 15) {
      parts.push(`${String(prefs.minAge ?? '')}-${String(prefs.maxAge ?? '')}y`)
    }
    if (prefs.sizes.length < 4) {
      parts.push(`${String(prefs.sizes.length ?? '')} sizes`)
    }
    if (prefs.maxDistance !== 50) {
      parts.push(`${String(prefs.maxDistance ?? '')}km`)
    }
    if (prefs.personalities.length > 0) {
      parts.push(`${String(prefs.personalities.length ?? '')} traits`)
    }
