...
export function PricingModal({ open, onOpenChange, onSuccess }: PricingModalProps) {
  ...
  const handleSubscribe = async (plan: PlanDefinition) => {
    ...
    try {
      ...
      toast.success(plan.trialDays ? `${String(plan.trialDays ?? '')}-day trial started!` : 'Subscription activated!', {
        description: `Welcome to ${String(plan.name ?? '')}! Your features are now unlocked.`
      })
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
            ...
            return (
              ...
              >
                ...
                <ul className="space-y-3 mb-6">
                  {plan.entitlements.length === 0 ? (
                    <li className="flex items-start gap-2 text-sm">
                      <Check size={20} weight="bold" className="text-muted-foreground shrink-0 mt-0.5" />
                ...
                <Button
                  ...
                  size="lg"
                >
                  {isLoading ? 'Processing...' : plan.tier === 'free' ? 'Current Plan' : 'Get Started'}
                </Button>

                {plan.tier !== 'free' && (
                  <p className="text-xs text-center text-muted-foreground mt-4">
                    Cancel anytime • Auto-renews {billingCycle === 'monthly' ? 'monthly' : 'annually'}
                  </p>
                )}
              </div>
            )
        ...
        <div className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <p className="mt-1">Questions? <button className="text-primary hover:underline">Contact Support</button></p>
        </div>
      </DialogContent>
    </Dialog>
  )
