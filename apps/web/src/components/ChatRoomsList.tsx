              ...
              <div className="flex items-start gap-3 relative z-10">
                <div className="relative shrink-0">
                  <MotionView
                    ...
                    transition={{ duration: 2, repeat: Infinity }}
                  >
                    <Avatar className={`w-14 h-14 ${String(hasUnread ? 'ring-2 ring-primary' : 'ring-2 ring-white/30' ?? '')}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className={`font-semibold truncate ${String(hasUnread ? 'text-foreground' : 'text-foreground/90' ?? '')}`}>
                      {room.matchedPetName || 'Unknown Pet'}
                    </h3>
                    {room.lastMessage && (
                      <span className={`text-xs shrink-0 ml-2 ${String(hasUnread ? 'text-primary font-semibold' : 'text-muted-foreground' ?? '')}`}>
                  ...
                  <div className="flex items-center gap-2">
                    {room.lastMessage ? (
                      <div className="flex-1 min-w-0 flex items-center gap-1">
                        ...
                        <p className={`text-sm truncate ${String(hasUnread ? 'text-foreground font-medium' : 'text-muted-foreground' ?? '')}`}>
                          {room.lastMessage.type === 'text' 
                            ? room.lastMessage.content 
                            : room.lastMessage.type === 'sticker'
                            ? `${String(room.lastMessage.content ?? '')} Sticker`
