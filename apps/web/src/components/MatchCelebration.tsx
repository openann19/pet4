...
function Particle({ index, total }: ParticleProps) {
  ...
  useEffect(() => {
    ...
    particleScale.value = withDelay(index * 20, withSequence(
      withTiming(1.5, { duration: 666 }),
      withTiming(0, { duration: 1334 })
    ))
    particleOpacity.value = withDelay(index * 20, withSequence(
      withTiming(1, { duration: 666 }),
      withTiming(0, { duration: 1334 })
    ))
  ...
  const particleStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotate: `${String(particleRotate.value ?? '')}deg` }
    ],
...
export default function MatchCelebration({ show, petName1, petName2, onComplete }: MatchCelebrationProps) {
  const presence = useAnimatePresence(show)
  ...
  const modalStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotate: `${String(modalRotate.value ?? '')}deg` }
    ]
  })) as AnimatedStyle
  ...
  const heartStyle = useAnimatedStyle(() => ({
    transform: [
      ...
      { rotate: `${String(heartRotate.value ?? '')}deg` }
    ]
  })) as AnimatedStyle
  ...
  const sparkle1Style = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(sparkle1Rotate.value ?? '')}deg` }]
  })) as AnimatedStyle

  const sparkle2Style = useAnimatedStyle(() => ({
    transform: [{ rotate: `${String(sparkle2Rotate.value ?? '')}deg` }]
  })) as AnimatedStyle
