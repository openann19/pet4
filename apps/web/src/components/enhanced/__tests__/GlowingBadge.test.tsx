...
describe('GlowingBadge', () => {
  ...
  it('should handle all variant types', () => {
    const variants: Array<'primary' | 'secondary' | 'accent' | 'success' | 'warning'> = [
      'primary',
      'secondary',
      'accent',
      'success',
    ...
    variants.forEach(variant => {
      const { unmount } = render(
        <GlowingBadge variant={variant}>{variant}</GlowingBadge>
      )
      const badge = screen.getByRole('status')
      expect(badge).toBeInTheDocument()
      unmount()
  ...
  it('should be accessible with screen readers', () => {
    render(
      <GlowingBadge aria-label="Notification badge">
        New
      </GlowingBadge>
    )
