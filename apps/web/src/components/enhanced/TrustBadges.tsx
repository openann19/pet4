...
export function TrustScore({ score, size = 'md', showLabel = false }: TrustScoreProps) {
  ...
  const circleAnimatedStyle = useAnimatedStyle(() => {
    return {
      strokeDasharray: `${String(strokeDasharray.value ?? '')} ${String(circumference ?? '')}`,
    }
  })
    ...
    <div className="flex items-center gap-2">
      <div className="relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`font-bold ${String(sizeClasses[size] ?? '')} ${String(getScoreColor(score) ?? '')}`}>
            {score}
          </span>
        </div>
      </div>
      {showLabel && (
        <div className="flex flex-col">
          <span className={`font-semibold ${String(sizeClasses[size] ?? '')}`}>
            {getScoreLabel(score)}
          </span>
