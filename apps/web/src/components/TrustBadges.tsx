...
const badgeIcons: Record<TrustBadge['type'], React.ComponentType<any>> = {
              ...
              <Tooltip key={badge.id}>
                <TooltipTrigger asChild>
                  <Badge
                    variant="outline"
                    className={cn(
                      'flex items-center gap-1.5 px-2.5 py-1 transition-all duration-300 hover:scale-105 cursor-default animate-in fade-in zoom-in',
                      colorClass
                    )}
            ...
            <Tooltip key={badge.id}>
              <TooltipTrigger asChild>
                <Badge
                  variant="outline"
                  className={cn(
                    'flex items-center justify-center w-8 h-8 p-0 rounded-full transition-all duration-300 hover:scale-110 cursor-default animate-in fade-in zoom-in',
                    compact && 'w-7 h-7',
                    colorClass
                  )}
                  style={{ animationDelay: `${index * 50}ms` }}
