...
const k = (n: number): string => (n >= 1000 ? `${String((n / 1000).toFixed(n % 1000 === 0 ? 0 : 1) ?? '')}K` : String(n ?? ''))
...
export default function PetsDemoPage(): ReactElement {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>): void => {
    ...
    if (target.src !== `${String(window.location.origin ?? '')}/demo/gradient.jpg`) {
  ...
  const handleAvatarError = (e: React.SyntheticEvent<HTMLImageElement>): void => {
    ...
    if (target.src !== `${String(window.location.origin ?? '')}${String(fallbackSrc ?? '')}`) {
              ...
              {[
                ...
                <button
                  ...
                  className={`h-9 px-4 rounded-full text-sm font-medium transition ${
                    String(i === 1 ? 'text-white grad-brand shadow-[0_0_24px_rgba(255,82,168,.35)]' : 'text-white/70 hover:text-white/90 hover:bg-white/10' ?? '')
                  }`}
        ...
        <div className="pointer-events-auto px-2 sm:px-4 pb-[env(safe-area-inset-bottom,0px)]">
          <div className="glass h-14 rounded-xl flex items-center justify-around text-white/85">
            {['Home','Discover','Messages','Wallet','Profile'].map((label, i) => (
              <button 
                key={label} 
                className={`flex flex-col items-center justify-center h-full w-16 rounded-lg ${String(i===0 ? 'text-white' : 'hover:text-white' ?? '')}`}
                type="button"
              >
