...
export function useBubbleHoverTilt(
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      transform: [
        { rotateX: `${String(tiltY.value ?? '')}deg` },
        { rotateY: `${String(-tiltX.value ?? '')}deg` },
        { translateZ: `${String(lift.value ?? '')}px` }
      ],
      boxShadow: `0 ${String(Math.abs(lift.value) ?? '')}px ${String(shadowRadius ?? '')}px rgba(59, 130, 246, ${String(glowOpacity.value * 0.4 ?? '')})`
    }
  })
  ...
  const glowStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        { translateZ: `${String(lift.value * 1.5 ?? '')}px` }
      ]
    }
  })
