  ...
  const enterFullscreen = useCallback(async () => {
    ...
    try {
      if (element.requestFullscreen) {
        ...
        typeof (element as { webkitRequestFullscreen?: () => Promise<void> })
          .webkitRequestFullscreen === 'function'
      ) {
        await (
          element as { webkitRequestFullscreen: () => Promise<void> }
        ).webkitRequestFullscreen();
      } else if (
        'mozRequestFullScreen' in element &&
        typeof (element as { mozRequestFullScreen?: () => Promise<void> }).mozRequestFullScreen ===
          'function'
      ) {
        await (element as { mozRequestFullScreen: () => Promise<void> }).mozRequestFullScreen();
      } else if (
        'msRequestFullscreen' in element &&
        typeof (element as { msRequestFullscreen?: () => Promise<void> }).msRequestFullscreen ===
          'function'
      ) {
  ...
  const exitFullscreen = useCallback(async () => {
    try {
      if (document.exitFullscreen) {
        ...
        typeof (document as { webkitExitFullscreen?: () => Promise<void> }).webkitExitFullscreen ===
          'function'
      ) {
        await (document as { webkitExitFullscreen: () => Promise<void> }).webkitExitFullscreen();
      } else if (
        'mozCancelFullScreen' in document &&
        typeof (document as { mozCancelFullScreen?: () => Promise<void> }).mozCancelFullScreen ===
          'function'
      ) {
        await (document as { mozCancelFullScreen: () => Promise<void> }).mozCancelFullScreen();
      } else if (
        'msExitFullscreen' in document &&
        typeof (document as { msExitFullscreen?: () => Promise<void> }).msExitFullscreen ===
          'function'
      ) {
