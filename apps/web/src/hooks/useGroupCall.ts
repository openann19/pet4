...
export function useGroupCall(
  ...
  const initiateGroupCall = async (
    ...
    try {
      ...
      if (isTruthy(actualResolution)) {
        toast.success(`Group call starting with ${String(actualResolution ?? '')}`)
      }
      ...
      logger.error('Failed to start group call', error instanceof Error ? error : new Error(String(error)))
    }
  }
  ...
  const addParticipant = (participant: {
    ...
    toast.success(`${String(participant.name ?? '')} joined the call`)
  }
  ...
  const removeParticipant = (participantId: string) => {
    ...
    toast.info(`${String(participant.name ?? '')} left the call`)
  }
