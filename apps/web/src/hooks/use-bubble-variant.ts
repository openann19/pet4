...
export function useBubbleVariant(options: UseBubbleVariantOptions): UseBubbleVariantReturn {
  const { variant, delay = DEFAULT_DELAY, enabled = true } = options;
  ...
  const trigger = useCallback(() => {
    if (variant === 'ai-answer') {
      ...
      tilt.value = withDelay(delay, withSpring(2, springConfigs.smooth));
  ...
  const animatedStyle = useAnimatedStyle(() => {
    const rotateZ = interpolate(tilt.value, [-5, 5], [-5, 5], Extrapolation.CLAMP);
