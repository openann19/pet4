...
export function useNativeSwipe(
  options: UseNativeSwipeOptions
): UseNativeSwipeReturn {
  const {
    cardWidth,
  ...
  const triggerHaptic = useCallback((type: 'selection' | 'light' | 'medium' | 'success') => {
  ...
  const updateAnimation = useCallback((translationXValue: number, translationYValue: number) => {
    const clampedX = Math.max(-cardWidth * 1.5, Math.min(cardWidth * 1.5, translationXValue))
    const clampedY = Math.max(-MAX_VERTICAL_DRAG, Math.min(MAX_VERTICAL_DRAG, translationYValue * 0.3))
  ...
  const handleCommit = useCallback((direction: 'left' | 'right', velocity: number) => {
    if (isTruthy(isCommittedRef.current)) return
    isCommittedRef.current = true

    triggerHaptic(direction === 'right' ? 'success' : 'light')
    
    const exitX = direction === 'right' ? cardWidth * 1.2 : -cardWidth * 1.2
    const exitRotation = reduceMotion ? 0 : (direction === 'right' ? MAX_ROTATION + 5 : -(MAX_ROTATION + 5))
  ...
  const handleDrag = useCallback((translationXValue: number, translationYValue: number) => {
    if (isTruthy(isCommittedRef.current)) return
    updateAnimation(translationXValue, translationYValue)
  }, [updateAnimation])

  const handleDragEnd = useCallback((translationXValue: number, velocityX: number) => {
    if (isTruthy(isCommittedRef.current)) return
  ...
  }, [threshold, velocityThreshold, onDragEnd, handleCommit, translationX, translationY, rotation, likeOpacity, passOpacity, scale])
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const likeOpacityValue = interpolate(
      clampedX,
      [0, threshold],
      [0, 1],
      Extrapolation.CLAMP
    ...
    const passOpacityValue = interpolate(
      clampedX,
      [-threshold, 0],
      [1, 0],
      Extrapolation.CLAMP
    ...
    return {
      transform: [
        ...
        { rotate: `${String(rotationValue ?? '')}deg` },
        { scale: scaleValue }
      ],
