...
describe('useDeleteBubbleAnimation', () => {
  ...
  it('should trigger self-delete animation', () => {
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ context: 'self-delete' })
    )
  ...
  it('should trigger admin-delete animation', () => {
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ context: 'admin-delete' })
    )
  ...
  it('should trigger emoji-media delete animation', () => {
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ context: 'emoji-media' })
    )
  ...
  it('should trigger group-chat delete animation', () => {
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ context: 'group-chat' })
    )
  ...
  it('should call onFinish callback after animation', () => {
    ...
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ onFinish, duration: 300 })
    )
  ...
  it('should respect custom duration', () => {
    const onFinish = vi.fn()
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ onFinish, duration: 500 })
    )
  ...
  it('should work without haptic feedback', () => {
    const { result } = renderHook(() =>
      useDeleteBubbleAnimation({ hapticFeedback: false })
    )
  ...
  it('should handle all animation contexts', () => {
    const contexts: Array<'self-delete' | 'admin-delete' | 'emoji-media' | 'group-chat'> = [
      'self-delete',
      'admin-delete',
      'emoji-media',
      'group-chat'
    ...
    contexts.forEach((context) => {
      const { result } = renderHook(() =>
        useDeleteBubbleAnimation({ context })
      )
