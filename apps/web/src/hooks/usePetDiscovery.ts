...
export function usePetDiscovery({
  userPet,
  preferences = {},
  showAdoptableOnly = false,
  swipedPetIds,
}: UsePetDiscoveryOptions = {}) {
  ...
  const swipedIds = useMemo(() => {
    if (swipedPetIds) return swipedPetIds;
    ...
    return allPets
      .map((pet) => {
        ...
        if (userPet?.location && pet.location) {
          ...
          if (
            userCoords[0] !== undefined &&
            userCoords[1] !== undefined &&
            petCoords[0] !== undefined &&
            petCoords[1] !== undefined
          ) {
  ...
  const markAsSwiped = useCallback(
    (petId: string) => {
      if (!userPet) return;
      ...
      void setSwipeHistory((prev) => [...(prev ?? []), newSwipe]);
    },
