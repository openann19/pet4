...
export function useKeyboardNavigation(
  ...
  const shortcutsRef = useRef<
    Map<string, { shortcut: KeyboardShortcut; handler: (event: KeyboardEvent) => void }>
  >(new Map());
      ...
      switch (event.key) {
        case 'ArrowDown':
          if (orientation === 'vertical' || orientation === 'both') {
            ...
            nextIndex =
              wrap && currentIndex === focusableElements.length - 1
                ? 0
