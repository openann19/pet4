...
export function useOptimizedKV<T>(key: string, defaultValue: T) {
  ...
  const optimizedSetValue = useCallback((newValue: T | ((current: T) => T)) => {
    setValue(newValue)
  }, [setValue])
...
export function useDebouncedKV<T>(key: string, defaultValue: T, delay: number = 300) {
  const [value, setValue, deleteValue] = useStorage<T>(key, defaultValue)
  
  const debouncedSetValue = useCallback((newValue: T | ((current: T) => T)) => {
