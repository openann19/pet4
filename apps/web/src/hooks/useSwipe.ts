...
export function useSwipe({
  ...
  const handleDrag = useCallback((translationX: number) => {
    ...
    const threshold = 80
    if (hapticFeedback && Math.abs(translationX) > threshold && Math.abs(translationX) < threshold + 20) {
  ...
  }, [swipeThreshold, swipeVelocity, hapticFeedback, onSwipe, x, rotate, opacity, likeOpacity, passOpacity])
