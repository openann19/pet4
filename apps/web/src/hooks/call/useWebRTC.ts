...
export function useWebRTC(options: UseWebRTCOptions) {
        ...
        peerConnection.initialize({
          ...
          onIceConnectionStateChange: (state) => {
            ...
            if (state === 'connected' || state === 'completed') {
              ...
              setCallState(prev => ({
                ...prev,
                error: `ICE connection ${String(state ?? '')}`,
                isConnected: false
              }))
            }
          },
  ...
  const cleanup = useCallback(async () => {
    // Stop local stream
    if (isTruthy(localStreamRef.current)) {
      mediaStreamManager.stopStream(localStreamRef.current)
      localStreamRef.current = null
    }

    // Close peer connection
    if (isTruthy(peerConnectionRef.current)) {
      await peerConnectionRef.current.close()
      peerConnectionRef.current = null
    }

    // Cleanup signaling
    if (isTruthy(signalingClientRef.current)) {
