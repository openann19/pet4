...
function useButtonAnimations(
  ...
  useEffect(() => {
    ...
    switch (state) {
      ...
      case 'success':
        ...
        // Haptic feedback
        if (enableHaptics && feedbackHaptics) {
  ...
  return { scale, opacity, glowIntensity, colorShift, spinnerRotation };
}
...
export function useButtonState(options: UseButtonStateOptions = {}): UseButtonStateReturn {
  ...
  // Animation values
  const { scale, opacity, glowIntensity, colorShift, spinnerRotation } = useButtonAnimations(
    state,
    setState,
    reducedMotion,
    enableHaptics,
