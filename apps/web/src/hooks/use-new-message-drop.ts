...
import { useSharedValue, useAnimatedStyle, withSpring, withSequence, withDelay, withTiming, interpolate, Extrapolation, type SharedValue } from 'react-native-reanimated'
...
export function useNewMessageDrop(
  options: UseNewMessageDropOptions = {}
): UseNewMessageDropReturn {
  const {
    isNew = false,
    delay = DEFAULT_DELAY,
  ...
  const glowStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      boxShadow: `0 0 ${String(radius ?? '')}px rgba(59, 130, 246, 0.6)`
    }
  })
