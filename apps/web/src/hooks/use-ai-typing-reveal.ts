...
import { useSharedValue, useAnimatedStyle, withTiming, withDelay, withSequence, withRepeat, interpolate, Extrapolation, type SharedValue } from 'react-native-reanimated'
...
export function useAITypingReveal(
  options: UseAITypingRevealOptions
): UseAITypingRevealReturn {
  const {
    text,
    enabled = true,
  ...
  const start = useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentIndex <= text.length) {
        revealedLength.value = withTiming(
          currentIndex,
  ...
  }, [enabled, text, typingSpeed, cursorBlinkSpeed, revealDelay, revealedLength, contentOpacity, cursorOpacity])
