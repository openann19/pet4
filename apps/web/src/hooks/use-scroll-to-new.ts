...
export function useScrollToNew(
  options: UseScrollToNewOptions = {}
): UseScrollToNewReturn {
  const {
    isVisible = false,
    containerRef,
    targetRef,
  ...
  const scrollToLatest = useCallback(() => {
    if (isTruthy(isScrollingRef.current)) return
    ...
    } else if (isTruthy(targetRef?.current)) {
