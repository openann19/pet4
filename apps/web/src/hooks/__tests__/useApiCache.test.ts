...
describe('useApiCache', () => {
  ...
  it('fetches data when enabled', async () => {
    ...
    const { result } = renderHook(() =>
      useApiCache('test-key', mockFetcher, { enabled: true })
    )
  ...
  it('does not fetch when disabled', () => {
    const { result } = renderHook(() =>
      useApiCache('test-key', mockFetcher, { enabled: false })
    )
  ...
  it('calls onSuccess callback when data is fetched', async () => {
    ...
    renderHook(() =>
      useApiCache('test-key', mockFetcher, { onSuccess: mockOnSuccess })
    )
  ...
  it('calls onError callback when fetch fails', async () => {
    ...
    renderHook(() =>
      useApiCache('test-key', mockFetcher, { onError: mockOnError })
    )
  ...
  it('mutates data with TTL', () => {
    const newData = { data: 'mutated' }
    const { result } = renderHook(() =>
      useApiCache('test-key', mockFetcher, { ttl: 5000 })
    )
  ...
  it('refetches on mount when refetchOnMount is true', async () => {
    ...
    renderHook(() =>
      useApiCache('test-key', mockFetcher, { refetchOnMount: true })
    )
  ...
  it('does not refetch on mount when refetchOnMount is false', () => {
    ...
    renderHook(() =>
      useApiCache('test-key', mockFetcher, { refetchOnMount: false })
    )
