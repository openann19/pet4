...
class VerificationApiImpl {
  ...
  async getVerificationRequests(
    ...
    try {
      ...
      if (isTruthy(filters?.petId)) queryParams.append('petId', filters.petId)
      if (isTruthy(filters?.userId)) queryParams.append('userId', filters.userId)

      const url = `/verification/requests${String(queryParams.toString() ? `?${String(queryParams.toString() ?? '')}` : '' ?? '')}`
