...
export class CommunityAPI {
  ...
  async getPostById(id: string): Promise<Post | null> {
    try {
      ...
      try {
        await APIClient.post(`${String(ENDPOINTS.COMMUNITY.POST(id) ?? '')}/view`)
      } catch (error) {
  ...
  async reportContent(
    ...
    try {
      ...
      await APIClient.post(
        `${String(ENDPOINTS.COMMUNITY.POST(data.resourceId) ?? '')}/report`,
        request
      )
  ...
  async updatePostStatus(
    ...
    try {
      ...
      if (!isValidPostStatusTransition(currentPost.status, status)) {
        throw new Error(`Invalid status transition from ${String(currentPost.status ?? '')} to ${String(status ?? '')}`)
      }
