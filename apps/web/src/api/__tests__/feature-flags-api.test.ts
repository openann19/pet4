...
beforeAll(async () => {
  server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === 'GET' && url.pathname.includes('/ab-tests/') && url.pathname.includes('/variant')) {
      res.setHeader('Content-Type', 'application/json')
...
describe('FeatureFlagsAPI.updateFeatureFlag', () => {
  it('should update feature flag', async () => {
    const flag = await featureFlagsApi.updateFeatureFlag('stories_enabled', { enabled: false }, 'admin-1')
