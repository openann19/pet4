...
beforeAll(async () => {
  server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === 'GET' && url.pathname === '/kyc/status' && !url.searchParams.get('submissions')) {
      res.setHeader('Content-Type', 'application/json')
    ...
    if (req.method === 'GET' && url.pathname === '/kyc/status' && url.searchParams.get('submissions') === 'true') {
      res.setHeader('Content-Type', 'application/json')
      res.end(JSON.stringify({ data: [mockSubmission] }))
      return
    }
