...
class AdoptionApiImpl {
  ...
  async getAdoptionProfiles(
    ...
    try {
      ...
      if (isTruthy(filters?.status)) params.append('status', filters.status)
      if (isTruthy(filters?.breed)) params.append('breed', filters.breed)
      if (isTruthy(filters?.minAge)) params.append('minAge', String(filters.minAge))
      if (isTruthy(filters?.maxAge)) params.append('maxAge', String(filters.maxAge))
      if (filters?.size && filters.size.length > 0) {
        filters.size.forEach(s => { params.append('size', s); })
      }
      if (isTruthy(filters?.location)) params.append('location', filters.location)
      ...
      if (isTruthy(filters?.cursor)) params.append('cursor', filters.cursor)
      if (isTruthy(filters?.limit)) params.append('limit', String(filters.limit))
  ...
  async getUserApplications(userId: string): Promise<AdoptionApplication[]> {
    try {
      const response = await APIClient.get<AdoptionApplication[]>(
        `${String(ENDPOINTS.ADOPTION.APPLICATIONS ?? '')}?applicantId=${String(userId ?? '')}`
      )
      return response.data
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
  ...
  async getApplicationsByProfile(profileId: string): Promise<AdoptionApplication[]> {
    try {
      const response = await APIClient.get<AdoptionApplication[]>(
        `${String(ENDPOINTS.ADOPTION.APPLICATIONS ?? '')}?adoptionProfileId=${String(profileId ?? '')}`
      )
      return response.data
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
