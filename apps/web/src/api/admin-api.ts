...
class AdminApiImpl {
  ...
  async createAuditLog(entry: Omit<AuditLogEntry, 'id' | 'timestamp'>): Promise<void> {
    try {
      await APIClient.post(
        `${String(ENDPOINTS.ADMIN.SETTINGS ?? '')}/audit`,
        {
          ...entry,
  ...
  async getAuditLogs(limit: number = 100): Promise<AuditLogEntry[]> {
    try {
      const response = await APIClient.get<AuditLogEntry[]>(
        `${String(ENDPOINTS.ADMIN.SETTINGS ?? '')}/audit?limit=${String(limit ?? '')}`
      )
      return response.data
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
  ...
  async moderatePhoto(
    ...
    try {
      await APIClient.post(
        `${String(ENDPOINTS.ADMIN.SETTINGS ?? '')}/moderate`,
        {
          taskId,
          action,
  ...
  async getConfigHistory(
    ...
    try {
      ...
      if (isTruthy(configType)) params.append('type', configType)
      ...
      const response = await APIClient.get<Array<{
        ...
        timestamp: string
        changes: Record<string, unknown>
      }>>(`${String(ENDPOINTS.ADMIN.CONFIG_HISTORY ?? '')}?${String(params.toString() ?? '')}`)
      return response.data
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
