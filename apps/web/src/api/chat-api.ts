...
class ChatApiImpl {
  ...
  async getMessages(
    ...
    try {
      ...
      const response = await APIClient.get<GetMessagesResponse>(
        `${String(ENDPOINTS.CHAT.MESSAGES(roomId) ?? '')}${String(query ?? '')}`
      )
      return response.data
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
  ...
  async markAsRead(
    ...
    try {
      await APIClient.post(
        String(ENDPOINTS.CHAT.MARK_READ(roomId) ?? ''),
        { messageId }
      )
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
  ...
  async sendTypingIndicator(
    ...
    try {
      await APIClient.post(
        `/api/v1/chat/rooms/${String(roomId ?? '')}/typing`,
        request
      )
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
  ...
  async deleteMessage(
    ...
    try {
      await APIClient.delete(
        `/api/v1/chat/messages/${String(messageId ?? '')}`,
        {
   ...
   */
  async getConversations(): Promise<Array<{ id: string; participants: string[]; lastMessage?: Message; updatedAt: string }>> {
    try {
      const response = await APIClient.get<Array<{ id: string; participants: string[]; lastMessage?: Message; updatedAt: string }>>(
   ...
   */
  async getConversation(conversationId: string): Promise<{ id: string; participants: string[]; createdAt: string }> {
    try {
      const response = await APIClient.get<{ id: string; participants: string[]; createdAt: string }>(
