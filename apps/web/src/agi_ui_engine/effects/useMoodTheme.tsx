...
export function useMoodTheme(
  ...
  const gradientColors = useMemo(() => {
    ...
    if (isTruthy(hasPositive)) {
      return [
        `rgba(110, 231, 183, ${String(0.15 * theme.gradientIntensity ?? '')})`,
        `rgba(139, 92, 246, ${String(0.1 * theme.gradientIntensity ?? '')})`
      ]
    }

    if (isTruthy(hasNegative)) {
      return [
        `rgba(239, 68, 68, ${String(0.15 * theme.gradientIntensity ?? '')})`,
        `rgba(251, 146, 60, ${String(0.1 * theme.gradientIntensity ?? '')})`
      ]
    }

    if (isTruthy(hasNeutral)) {
      return [
        `rgba(148, 163, 184, ${String(0.15 * theme.gradientIntensity ?? '')})`,
        `rgba(203, 213, 225, ${String(0.1 * theme.gradientIntensity ?? '')})`
      ]
    }

    return [
      `rgba(59, 130, 246, ${String(0.15 * theme.gradientIntensity ?? '')})`,
      `rgba(147, 197, 253, ${String(0.1 * theme.gradientIntensity ?? '')})`
    ]
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      background: `linear-gradient(135deg, ${String(gradientColors[0] ?? '')}, ${String(gradientColors[1] ?? '')})`,                                                                        
