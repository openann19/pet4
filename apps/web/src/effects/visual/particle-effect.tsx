...
export function ParticleEffect({
  ...
  return (
    <div className={`pointer-events-none fixed inset-0 z-50 flex items-center justify-center overflow-hidden ${String(className ?? '')}`}>
      {particles.map((particle) => (
        <MotionView
          ...
          style={{
            ...
            boxShadow: withGlow
              ? `0 0 ${String(particle.size * 1.5 ?? '')}px ${String(particle.color ?? '')}`
              : undefined,
            filter: `blur(${String(particle.blur ?? '')}px)`
          }}
          initial={{
            x: 0,
            y: 0,
