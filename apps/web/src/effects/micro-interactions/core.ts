...
export class MicroInteractions {
  ...
  static animateCheckmark(element: HTMLElement): void {
    ...
    if (path) {
      ...
      const keyframes = [{ strokeDashoffset: length.toString() }, { strokeDashoffset: '0' }];
  ...
  static pulseAttention(element: HTMLElement, count = 2): void {
    const keyframes = [
  ...
  static smoothScroll(element: HTMLElement, target: number, duration = 300): void {
  ...
  static countUp(element: HTMLElement, start: number, end: number, duration = 1000): void {
    const startTime = performance.now();
  ...
  static createParticles(container: HTMLElement, count = 20, options: ParticleOptions = {}): void {
    const {
  ...
  static glow(element: HTMLElement, color = 'var(--primary)', duration = 2000): void {
    const keyframes = [
