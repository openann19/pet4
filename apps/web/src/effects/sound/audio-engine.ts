  ...
  private async playPreset(
    ...
    try {
      ...
      if (preset.type === 'oscillator') {
        ...
        if (isTruthy(preset.frequencyEnvelope)) {
      ...
      if (isTruthy(preset.volumeEnvelope)) {
...
if (typeof window !== 'undefined') {
  audioEngine.initialize().catch(error => {
    logger.warn('Failed to auto-initialize audio engine', { error: error instanceof Error ? error.message : String(error) })
  })
}
