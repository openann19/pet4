  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      background: `linear-gradient(135deg, 
        rgba(${String(primaryR ?? '')}, ${String(primaryG ?? '')}, ${String(primaryB ?? '')}, ${String(intensity ?? '')}) 0%, 
        rgba(${String(secondaryR ?? '')}, ${String(secondaryG ?? '')}, ${String(secondaryB ?? '')}, ${String(intensity * 0.8 ?? '')}) 100%
      )`,
      boxShadow: `0 4px ${String(shadowBlur ?? '')}px ${String(shadowOpacity ?? '')}px ${String(themeColor.shadow ?? '')}`
    }
  })
