...
export function useReceiptTransition(
  ...
  const animateStatusChange = useCallback((newStatus: MessageStatus) => {
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const r = interpolate(
      ...
      [0, 1],
      [
        parseInt(baseColor.match(/rgba?\((\d+)/)?.[1] ?? '156', 10),
        parseInt(targetColor.match(/rgba?\((\d+)/)?.[1] ?? '156', 10)
      ],
      Extrapolation.CLAMP
    )
    const g = interpolate(
      ...
      [0, 1],
      [
        parseInt(baseColor.match(/rgba?\(\d+, (\d+)/)?.[1] ?? '163', 10),
        parseInt(targetColor.match(/rgba?\(\d+, (\d+)/)?.[1] ?? '163', 10)
      ],
      Extrapolation.CLAMP
    )
    const b = interpolate(
      ...
      [0, 1],
      [
        parseInt(baseColor.match(/rgba?\(\d+, \d+, (\d+)/)?.[1] ?? '175', 10),
        parseInt(targetColor.match(/rgba?\(\d+, \d+, (\d+)/)?.[1] ?? '175', 10)
      ],
      Extrapolation.CLAMP
    )
