...
export function useStatusTicks(options: UseStatusTicksOptions): UseStatusTicksReturn {
  ...
  const { feedback, theme: _theme } = useUIConfig();
  ...
  const updateStatus = useCallback(() => {
    ...
    tick1Fill.value = withTiming(newTick1Fill, {
      duration,
      easing: isReducedMotion ? Easing.linear : Easing.inOut(Easing.ease), // Smooth bezier for morphing
    });
    ...
    if (isOwnMessage && previousStatus === 'delivered' && status === 'read' && feedback.haptics) {
