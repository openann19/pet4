 ...
 */
const GLASS_ZOOM_EASING = Easing.inOut(Easing.cubic);
...
export function useGlassMorphZoom(options: UseGlassMorphZoomOptions = {}): UseGlassMorphZoomReturn {
  ...
  const { visual, feedback, _animation } = useUIConfig();
  ...
  const open = useCallback(() => {
    ...
    const baseDuration = ZOOM_DURATION_MIN + randomRange(0, ZOOM_DURATION_MAX - ZOOM_DURATION_MIN);
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const filter =
      visual.enableGlow && bloomIntensity.value > 0
