...
export async function createVerificationNotification(status: 'approved' | 'rejected', petName: string) {
  return addNotification({
    ...
    message: status === 'approved'
      ? `${String(petName ?? '')} has been verified and is now visible to other users!`
      : `${String(petName ?? '')}'s verification needs more information. Please check your profile.`,
...
export async function createStoryNotification(userName: string, count: number = 1) {
  return addNotification({
    ...
    message: count > 1
      ? `${String(userName ?? '')} and ${String(count - 1 ?? '')} others posted new stories`
      : `${String(userName ?? '')} posted a new story`,
    priority: 'low',
    actionUrl: '/stories',
    metadata: {
      userName,
