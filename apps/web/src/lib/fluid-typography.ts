...
export function getFluidTypographyClasses(variant: FluidTypographyVariant): string {
  ...
  return `${String(style.base ?? '')} ${String(style.weight ?? '')} ${String(style.lineHeight ?? '')} ${String(style.tracking ?? '')}`
}
...
export function getButtonSizeClasses(size: ButtonSize): string {
  ...
  return `${String(sizeConfig.padding ?? '')} ${String(sizeConfig.minWidth ?? '')} ${String(sizeConfig.maxWidth ?? '')} ${String(sizeConfig.height ?? '')} ${String(sizeConfig.typography ?? '')}`
}
