...
export class VerificationService {
  static createVerificationRequest(
    petId: string,
    userId: string,
  ...
  static submitForReview(request: VerificationRequest): VerificationRequest {
    return {
      ...request,
      status: 'pending',
    };
  ...
  static processReview(request: VerificationRequest, approve = true): VerificationRequest {
    // Real verification processing - no artificial delay
    const now = new Date().toISOString();
    const trustScore = approve
  ...
  static checkExpiredVerifications(requests: VerificationRequest[]): VerificationRequest[] {
    const now = new Date();
