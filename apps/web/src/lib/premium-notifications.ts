...
export async function createLikeNotification(
  ...
  return createPremiumNotification({
    type: 'like',
    title: count === 1 ? `${String(petName ?? '')} liked your pet!` : `${String(count ?? '')} new likes!`,
    message: count === 1 
      ? `${String(petName ?? '')} is interested in your pet` 
      : `${String(petName ?? '')} and ${String(count - 1 ?? '')} others liked your pet`,
    priority: 'normal',
...
export async function createVerificationNotification(
  ...
  const messages = {
    approved: {
      title: 'Pet Verified! ✅',
      message: `${String(petName ?? '')} has been verified and is now live`,
      priority: 'high' as const
    },
    rejected: {
      title: 'Verification Issue',
      message: `${String(petName ?? '')}'s verification needs attention`,
      priority: 'urgent' as const
    },
    pending: {
      title: 'Verification Pending',
      message: `${String(petName ?? '')} is being reviewed`,
...
export async function createStoryNotification(
  ...
  return createPremiumNotification({
    type: 'story',
    title: count === 1 ? `${String(userName ?? '')} posted a story` : `${String(count ?? '')} new stories`,
    message: count === 1
      ? 'View their latest update'
      : `${String(userName ?? '')} and others shared new stories`,
    priority: 'low',
    actionLabel: 'Watch',
    ...(avatarUrl ? { avatarUrl } : {}),
    ...(imageUrl ? { imageUrl } : {}),
