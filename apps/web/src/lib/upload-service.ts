...
class UploadServiceImpl {
  ...
  validateFile(file: File, type: 'image' | 'video' | 'document'): void {
    ...
    if (file.size > limit.maxSize) {
      throw new Error(`File size ${String(Math.round(file.size / 1024 / 1024) ?? '')}MB exceeds limit of ${String(Math.round(limit.maxSize / 1024 / 1024) ?? '')}MB`)
    }

    if (!limit.types.includes(file.type)) {
      throw new Error(`File type ${String(file.type ?? '')} is not supported. Allowed: ${String(limit.types.join(', ') ?? '')}`)
    }
  }
