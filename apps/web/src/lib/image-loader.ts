...
class ImageLoader {
  ...
  lazyLoad(
    ...
    const { placeholder, rootMargin = '50px', threshold = 0.01, onLoad, onError } = options;
  ...
  generateSrcSet(baseUrl: string, widths: number[], format?: string): string {
    return widths
      .map((width) => {
        const url = format ? `${baseUrl}?w=${width}&fm=${format}` : `${baseUrl}?w=${width}`;
        return `${url} ${width}w`;
      })
      .join(', ');
  }
...
export function generateBlurDataURL(src: string, size = 10): Promise<string> {
