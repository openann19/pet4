...
export function getAspectRatio(dimensions: ImageDimensions): string {
  ...
  return `${String(dimensions.width / divisor ?? '')}:${String(dimensions.height / divisor ?? '')}`
}
...
export function calculateResponsiveSizes(
  ...
  return breakpoints
    ...
    .map(bp => `(max-width: ${String(bp ?? '')}px) ${String(bp ?? '')}px`)
    .join(', ') + `, ${String(originalWidth ?? '')}px`
}
...
export function generatePlaceholderDataUrl(
  ...
  `
  return `data:image/svg+xml;base64,${String(btoa(svg) ?? '')}`
}
