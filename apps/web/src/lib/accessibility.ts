...
export function getReadableTimestamp(timestamp: string): string {
  ...
  if (diffMins < 60) return `${String(diffMins ?? '')} ${String(diffMins === 1 ? 'minute' : 'minutes' ?? '')} ago`
  if (diffHours < 24) return `${String(diffHours ?? '')} ${String(diffHours === 1 ? 'hour' : 'hours' ?? '')} ago`
  if (diffDays < 7) return `${String(diffDays ?? '')} ${String(diffDays === 1 ? 'day' : 'days' ?? '')} ago`
