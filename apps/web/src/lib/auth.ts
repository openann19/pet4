...
export class AuthService {
  ...
  async login(credentials: LoginCredentials): Promise<{ user: User; tokens: AuthTokens }> {
    ...
    if (user.passwordHash && user.passwordSalt) {
      ...
      log.warn('User without password hash - allowing login for migration', { context: 'AuthService', userId: user.id })
    }
  ...
  async refreshTokens(): Promise<AuthTokens | null> {
    if (isTruthy(this.refreshAttempted)) {
      return null
    }
  ...
  private generateTokens(user: User): AuthTokens {
    const accessToken = `access_${String(generateULID() ?? '')}_${String(user.id ?? '')}`
    const refreshToken = `refresh_${String(generateULID() ?? '')}_${String(user.id ?? '')}`
  ...
  hasPermission(_action: string, _resource: string): boolean {
    ...
    if (isTruthy(hasAdminRole)) return true
