...
export class SignalingClient {
  ...
  private setupEventHandlers(): void {
    ...
    this.realtimeClient.on('webrtc:signal', (data: {
      ...
      try {
        switch (data.type) {
          ...
          case 'answer':
            if (isTruthy(data.data)) {
              this.events.onAnswer?.(data.data as RTCSessionDescriptionInit, data.from, data.callId)
            }
            break
          case 'candidate':
            if (isTruthy(data.data)) {
              this.events.onIceCandidate?.(data.data as RTCIceCandidateInit, data.from, data.callId)
            }
            break
          case 'end':
  ...
  sendOffer(
    ...
    try {
      this.realtimeClient.emit('webrtc:signal', {
        type: 'offer',
