...
export class ModerationService {
  async getQueue(): Promise<ModerationQueue> {
    ...
    const queue =
      'queue' in response.data
        ? (response.data as ModerationQueueResponse).queue
        : (response.data as ModerationQueue)
  ...
  async takeTask(taskId: string, reviewerId: string): Promise<ModerationTask> {
    ...
    const task =
      'task' in response.data
        ? (response.data as ModerationTaskResponse).task
        : (response.data as ModerationTask)
  ...
  async getMetrics(): Promise<ModerationMetrics> {
    ...
    return 'metrics' in response.data
      ? (response.data as ModerationMetricsResponse).metrics
      : (response.data as ModerationMetrics)
  }
  ...
  private async fetchPhoto(photoId: string): Promise<PhotoRecord> {
    ...
    return 'photo' in response.data
      ? (response.data as PhotoResponse).photo
      : (response.data as PhotoRecord)
  }
  ...
  private async notifyUserDecision(
    ...
    await APIClient.post(ENDPOINTS.NOTIFICATIONS.LIST, {
      ...
      body: isApproved
        ...
        : `Your photo was not approved. Reason: ${decision.reasonText || decision.reason || 'See details in the app.'}`,
...
export class KYCService {
  ...
  async getUserSession(userId: string): Promise<KYCSession | null> {
    ...
    if ('session' in response.data) {
      return (response.data as KYCSessionResponse).session
    }
