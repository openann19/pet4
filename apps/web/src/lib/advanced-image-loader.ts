...
export class AdvancedImageLoader {
  ...
  static getOptimizedSrcSet(src: string, widths: number[] = [320, 640, 768, 1024, 1280, 1920]): string {
    return widths
      .map(width => `${String(this.getOptimizedSrc(src, width) ?? '')} ${String(width ?? '')}w`)
      .join(', ')
  }

  static getOptimizedSrc(src: string, width?: number, quality: number = 80): string {
    if (!width) return src
