...
export class VideoCompressor {
  ...
  static async compressVideo(
    ...
    if (metadata.duration > (opts.maxDurationSeconds || 60)) {
      throw new Error(`Video duration must be less than ${String(opts.maxDurationSeconds ?? '')} seconds`)
    }
        ...
        try {
          ...
          const captureFrame = () => {
            ...
            onProgress?.({
              stage: 'compressing',
              progress,
              message: `Processing frame ${String(frameCount ?? '')}...`
            })
  ...
  static formatFileSize(bytes: number): string {
    ...
    return `${String((bytes / Math.pow(k, i)).toFixed(2) ?? '')} ${String(sizes[i] ?? '')}`
  }
