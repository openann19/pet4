...
export async function searchNearbyPlaces(
  ...
  try {
    ...
    const url = new URL(`https://api.mapbox.com/geocoding/v5/mapbox.places/${String(searchQuery ?? '')}.json`)
    url.searchParams.set('proximity', `${String(location.lng ?? '')},${String(location.lat ?? '')}`)
    ...
    if (!response.ok) {
      ...
      logger.error('Mapbox API error', new Error(`HTTP ${String(response.status ?? '')}: ${String(errorText ?? '')}`), {
        status: response.status,
        statusText: response.statusText
      })
      throw new Error(`Mapbox API error: ${String(response.status ?? '')} ${String(response.statusText ?? '')}`)
    }
      ...
      .map(feature => {
        ...
        if (feature.context && feature.context.length > 0) {
          ...
          if (addressParts.length > 0) {
            address = `${String(feature.text ?? '')}, ${String(addressParts.join(', ') ?? '')}`
          }
        }
...
export async function searchPlacesByQuery(
  ...
  try {
    ...
    const url = new URL(`https://api.mapbox.com/geocoding/v5/mapbox.places/${String(encodedQuery ?? '')}.json`)
