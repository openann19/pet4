...
export const mapAPI = {
  ...
  async searchPlaces(query: string, location?: Location, radius?: number): Promise<SearchResult[]> {
    ...
    if (isTruthy(radius)) params.append('radius', radius.toString())

    const response = await fetch(`/api/map/search?${String(params ?? '')}`)
    return response.json()
  },
  ...
  async getPlaces(filters: {
    ...
    if (isTruthy(filters.category)) params.append('category', filters.category)
    ...
    if (isTruthy(filters.radius)) params.append('radius', filters.radius.toString())
    if (filters.verified !== undefined) params.append('verified', String(filters.verified))

    const response = await fetch(`/api/map/places?${String(params ?? '')}`)
    return response.json()
  },
  ...
  async deletePlace(id: string): Promise<void> {
    await fetch(`/api/map/places/${String(id ?? '')}`, { method: 'DELETE' })
  },

  async moderatePlace(id: string, status: 'approved' | 'rejected', reason?: string): Promise<Place> {
    const response = await fetch(`/api/map/places/${String(id ?? '')}/moderate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async getLostPetAlerts(location?: Location, radius?: number): Promise<LostPetAlert[]> {
    ...
    const response = await fetch(`/api/map/lost-pets?${String(params ?? '')}`)
    return response.json()
  },

  async createLostPetAlert(alert: Omit<LostPetAlert, 'id' | 'sightings' | 'createdAt'>): Promise<LostPetAlert> {
  ...
  async reportSighting(alertId: string, sighting: {
    location: Location
    description: string
    photo?: string
  }): Promise<LostPetAlert> {
    const response = await fetch(`/api/map/lost-pets/${String(alertId ?? '')}/sightings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async suggestPlaydateLocations(participants: string[], preferences?: {
    category?: string
    maxDistance?: number
  }): Promise<Place[]> {
  ...
  async createPlaydateRequest(request: Omit<PlaydateRequest, 'id' | 'createdAt'>): Promise<PlaydateRequest> {
  ...
  async geocode(address: string): Promise<Location> {
    ...
    const response = await fetch(`/api/geocode?${String(params ?? '')}`)
    return response.json()
  },
  ...
  async reverseGeocode(location: Location): Promise<string> {
    ...
    const response = await fetch(`/api/geocode/reverse?${String(params ?? '')}`)
  ...
  async deleteCategory(id: string): Promise<void> {
    await fetch(`/api/map/categories/${String(id ?? '')}`, { method: 'DELETE' })
  },
}
