...
Return as JSON with a "reasons" array of 2-3 strings:
{
  ...
  try {
    ...
    const data = JSON.parse(result) as { reasons?: string[] };
    return data.reasons ?? generateFallbackReasoning(userPet, otherPet, factors);
  } catch (_error) {
    const err = _error instanceof Error ? _error : new Error(String(_error));
    logger.warn('AI reasoning unavailable, using fallback logic', { _error: err });
...
function generateFallbackReasoning(
  ...
  if (factors.personalityMatch > 0.7) {
    ...
    if (commonTraits.length > 0 && commonTraits[0]) {
      reasoning.push(`Compatible personalities: both are ${commonTraits[0].toLowerCase()}`);
    }
  }
  ...
  if (factors.interestMatch > 0.5) {
    ...
    if (commonInterests.length > 0) {
      reasoning.push(
        `Share ${commonInterests.length} common interest${commonInterests.length > 1 ? 's' : ''}: ${commonInterests[0]}`
      );
    }
  }
