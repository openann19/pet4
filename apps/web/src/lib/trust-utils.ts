...
export function generateTrustBadges(petId: string, verified: boolean): TrustBadge[] {
  ...
  const possibleBadges: Array<Omit<TrustBadge, 'id' | 'earnedAt'>> = [
    {
      type: 'vaccinated',
  ...
  const selectedBadges = possibleBadges
...
export function generateRatings(petId: string, count: number = 5): Rating[] {
  ...
  const categories: Array<'playdate' | 'behavior' | 'temperament' | 'owner_communication' | 'general'> = [
    'playdate',
    'behavior',
    'temperament',
    'owner_communication',
...
export function calculateTrustProfile(ratings: Rating[], badges: TrustBadge[], playdateCount?: number): PetTrustProfile {
  ...
  const responseTime = responseTimeOptions[Math.floor(Math.random() * responseTimeOptions.length)] ?? '<1h'
