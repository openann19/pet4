...
class StorageService {
      ...
      this.broadcastChannel.onmessage = (event) => {
        ...
        if (type === 'invalidate') {
          if (isTruthy(key)) {
            ...
            logger.debug(`Cache invalidated for key: ${String(key ?? '')}`)
          } else {
          ...
          if (isTruthy(key)) {
            this.inMemoryCache.delete(key)
            logger.debug(`Cache invalidated for key: ${String(key ?? '')} (external update)`)
          } else {
  ...
  private getFromLocalStorage<T>(key: string): T | null {
    try {
      const item = localStorage.getItem(`${String(LOCAL_STORAGE_PREFIX ?? '')}${String(key ?? '')}`)
      if (!item) return null
      return JSON.parse(item) as T
    } catch (error) {
      logger.warn(`Failed to read from localStorage for key ${String(key ?? '')}`, { error: error instanceof Error ? error.message : String(error) })
      return null
    }
  }
  ...
  private setToLocalStorage<T>(key: string, value: T): void {
    try {
      localStorage.setItem(`${String(LOCAL_STORAGE_PREFIX ?? '')}${String(key ?? '')}`, JSON.stringify(value))
    } catch (error) {
      logger.warn(`Failed to write to localStorage for key ${String(key ?? '')}`, { error: error instanceof Error ? error.message : String(error) })
    }
  }
  ...
  private deleteFromLocalStorage(key: string): void {
    try {
      localStorage.removeItem(`${String(LOCAL_STORAGE_PREFIX ?? '')}${String(key ?? '')}`)
    } catch (error) {
      logger.warn(`Failed to delete from localStorage for key ${String(key ?? '')}`, { error: error instanceof Error ? error.message : String(error) })
    }
  }
  ...
  private async getFromIndexedDB<T>(key: string): Promise<T | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = store.get(key)
        ...
        const errorObj = error instanceof Error ? error : new Error(String(error))
        logger.error(`Failed to read from IndexedDB for key ${String(key ?? '')}`, errorObj)                                 
  ...
  private async setToIndexedDB<T>(key: string, value: T): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = store.put(item)
        ...
        const errorObj = error instanceof Error ? error : new Error(String(error))
        logger.error(`Failed to write to IndexedDB for key ${String(key ?? '')}`, errorObj)                                  
  ...
  async get<T = unknown>(key: string): Promise<T | undefined> {
    ...
    if (this.shouldUseLocalStorage(key)) {
      ...
      try {
        ...
        logger.warn(`Failed to read from IndexedDB for key ${String(key ?? '')}, falling back to localStorage`, { error: error instanceof Error ? error.message : String(error) })
        value = this.getFromLocalStorage<T>(key)
      }
    }
    ...
    if (this.shouldUseLocalStorage(key) || size < MAX_LOCALSTORAGE_SIZE) {
      ...
      if (size >= MAX_LOCALSTORAGE_SIZE) {
        try {
          ...
          logger.warn(`Failed to write to IndexedDB for key ${String(key ?? '')}`, { error: error instanceof Error ? error.message : String(error) })                         
        }
      }
      // Broadcast invalidation to other tabs
      ...
      try {
        ...
        logger.warn(`Failed to write to IndexedDB for key ${String(key ?? '')}, falling back to localStorage`, { error: error instanceof Error ? error.message : String(error) })                                                                             
        this.setToLocalStorage(key, value)
        // Broadcast invalidation to other tabs
        this.broadcastInvalidation(key)
      }
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      ...
      logger.warn(`Failed to delete from IndexedDB for key ${String(key ?? '')}`, { error: error instanceof Error ? error.message : String(error) })                          
    }
