...
class PermissionsManager {
  ...
  async requestCameraPermission(): Promise<boolean> {
    try {
      ...
      logger.error('Camera permission denied', error instanceof Error ? error : new Error(String(error)))
      return false
    }
  }
  ...
  async requestMicrophonePermission(): Promise<boolean> {
    try {
      ...
      logger.error('Microphone permission denied', error instanceof Error ? error : new Error(String(error)))
      return false
    }
  }
  ...
  async requestNotificationPermission(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('Notification permission denied', error instanceof Error ? error : new Error(String(error)))
      return false
    }
  }

  async requestPermissionWithRationale(request: PermissionRequest, language: 'en' | 'bg' = 'en'): Promise<boolean> {
    const status = await this.checkPermission(request.type)
    
    if (isTruthy(status.granted)) {
      request.onGrant?.()
      return true
    }

    if (isTruthy(status.permanent)) {
      request.onDeny?.()
      return false
    }
