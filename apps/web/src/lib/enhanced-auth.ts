...
export class EnhancedAuthService {
  ...
  private async createUserFromCurrentUser(user: User): Promise<UserProfile> {
    ...
    if (isTruthy((user as { isOwner?: boolean }).isOwner)) {
  ...
  private startSessionMonitoring(): void {
    if (isTruthy(this.sessionCheckInterval)) {
      clearInterval(this.sessionCheckInterval)
    }

    this.sessionCheckInterval = window.setInterval(async () => {
      if (isTruthy(this.currentSession)) {
  ...
  async updateUserProfile(updates: Partial<Omit<UserProfile, 'id' | 'createdAt' | 'githubId' | 'githubLogin'>>): Promise<UserProfile | null> {
  ...
  async cleanupExpiredSessions(): Promise<number> {
    ...
    for (const session of expiredSessions) {
      ...
      if (isTruthy(deleted)) deletedCount++
    }
