...
export async function checkRateLimit(
  ...
  try {
    ...
    const key = `rate-limit:${String(action ?? '')}:${String(userId ?? '')}`
...
export async function enforceRateLimit(
  ...
  if (!result.allowed) {
    ...
    throw new Error(
      `Rate limit exceeded. Maximum ${String(config.maxRequests ?? '')} ${String(config.action ?? '')} requests per ${String(Math.floor(config.windowMs / (60 * 1000)) ?? '')} minutes. ` +
      `Try again in ${String(resetMinutes ?? '')} minute${String(resetMinutes !== 1 ? 's' : '' ?? '')}.`
    )
  }
}
