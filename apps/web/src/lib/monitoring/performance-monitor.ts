...
class PerformanceMonitorImpl {
  ...
  private setupMemoryMonitoring(): void {
    ...
    setInterval(() => {
      ...
      if (isTruthy(performanceWithMemory.memory)) {
  ...
  trackAPICall(
    ...
    this.recordMetric({
      ...
      tags: {
        ...
        status_class: `${String(Math.floor(status / 100) ?? '')}xx`
      }
    })
  ...
  getSummary(): {
    ...
    const metricGroups = this.metrics.reduce<Record<string, number[]>>((groups, metric) => {
