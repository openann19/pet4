...
const kv = {
  get: vi.fn(async <T extends unknown>(key: string): Promise<T | undefined> => kvStore.get(key) as T | undefined),                                                              
...
describe('verifyReceipt', () => {
  ...
  describe('iOS platform (Apple)', () => {
    ...
    it('should handle errors for Apple receipt', async () => {
      vi.mocked(global.fetch).mockRejectedValueOnce(new Error('Apple API error'))
  ...
  describe('grantEntitlements', () => {
    it('should grant premium plan for premium SKU', async () => {
      ...
      const entitlements = kvStore.get('entitlements:user-123') as { plan: string; updatedAt: string }
...
describe('handleRefund', () => {
  ...
  it('should handle refund for existing purchase', async () => {
    ...
    kvStore.set(`purchase:${String(purchaseId ?? '')}`, purchase)

    await handleRefund(purchaseId, 'Customer request')

    const updatedPurchase = kvStore.get(`purchase:${String(purchaseId ?? '')}`) as Purchase
    expect(updatedPurchase?.status).toBe('refunded')

    const entitlements = kvStore.get(`entitlements:${String(userId ?? '')}`) as { plan: string; updatedAt: string }
    expect(entitlements?.plan).toBe('free')

    const auditLog = kvStore.get('audit-log:list') as Array<{
      id: string
      action: string
      userId: string
      purchaseId: string
    ...
    const refundEntry = auditLog.find(e => e.action === 'refund_processed' && e.purchaseId === purchaseId)
  ...
  it('should not downgrade if user is not on unlimited plan', async () => {
    ...
    const entitlements = kvStore.get(`entitlements:${String(userId ?? '')}`)
    expect(entitlements).toBeUndefined()
  })
  ...
  it('should limit audit log to 1000 entries', async () => {
    ...
    const auditLog = kvStore.get('audit-log:list') as Array<unknown>
  ...
  it('should handle refund without reason', async () => {
    ...
    kvStore.set(`purchase:${String(refundPurchaseId ?? '')}`, purchase)
    ...
    const refundEntry = auditLog.find(e => e.action === 'refund_processed' && e.purchaseId === refundPurchaseId)
    expect(refundEntry).toBeDefined()
