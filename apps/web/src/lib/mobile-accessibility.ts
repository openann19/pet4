...
class AccessibilityManager {
  ...
  ensureMinHitArea(element: HTMLElement): void {
    ...
    if (isTruthy(needsPadding)) {
      ...
      element.style.paddingTop = `${String(parseFloat(currentPaddingTop) + paddingY ?? '')}px`
      element.style.paddingRight = `${String(parseFloat(currentPaddingRight) + paddingX ?? '')}px`
      element.style.paddingBottom = `${String(parseFloat(currentPaddingBottom) + paddingY ?? '')}px`
      element.style.paddingLeft = `${String(parseFloat(currentPaddingLeft) + paddingX ?? '')}px`
  ...
  setDescription(element: HTMLElement, description: string, descriptionId?: string): void {
    const id = descriptionId || `desc-${String(Date.now() ?? '')}`
