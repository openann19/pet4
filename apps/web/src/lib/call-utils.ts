...
export function formatCallDuration(seconds: number): string {
  ...
  if (hours > 0) {
    return `${String(hours ?? '')}:${String(minutes.toString().padStart(2, '0') ?? '')}:${String(secs.toString().padStart(2, '0') ?? '')}`
  }
  return `${String(minutes ?? '')}:${String(secs.toString().padStart(2, '0') ?? '')}`
}
...
export function getActualResolution(stream: MediaStream): string {
  ...
  if (width >= 3840 && height >= 2160) {
    return `4K (${String(width ?? '')}x${String(height ?? '')} @ ${String(frameRate ?? '')}fps)`
  } else if (width >= 1920 && height >= 1080) {
    return `1080p (${String(width ?? '')}x${String(height ?? '')} @ ${String(frameRate ?? '')}fps)`
  } else if (width >= 1280 && height >= 720) {
    return `720p (${String(width ?? '')}x${String(height ?? '')} @ ${String(frameRate ?? '')}fps)`
  }
  return `${String(width ?? '')}x${String(height ?? '')} @ ${String(frameRate ?? '')}fps`
}
