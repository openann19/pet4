...
const _calculateCompatibilityScoreImpl = (pet1: Pet, pet2: Pet): number => {
  ...
  if (pet1.personality && Array.isArray(pet1.personality) && pet2.personality && Array.isArray(pet2.personality)) {
  ...
  if (pet1.interests && Array.isArray(pet1.interests) && pet2.interests && Array.isArray(pet2.interests)) {
  ...
  if (pet1.lookingFor && Array.isArray(pet1.lookingFor) && pet2.lookingFor && Array.isArray(pet2.lookingFor)) {
    const lookingForMatch = pet1.lookingFor.filter(goal => 
      pet2.lookingFor.includes(goal)
    ).length
