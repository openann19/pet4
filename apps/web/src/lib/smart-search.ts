...
export class SmartSearch<T> {
  ...
  search(query: string): SearchResult<T>[] {
    ...
    if (isTruthy(this.options.sortResults)) {
  ...
  private calculateScore(item: T, query: string): { score: number; matches: string[] } {
    ...
    for (const key of this.options.keys) {
      ...
      const normalizedValue = this.options.caseSensitive
        ? value
        : value.toLowerCase()
      ...
      if (normalizedValue.includes(query)) {
        ...
        let keyScore = 0
        if (isTruthy(exactMatch)) keyScore = 1.0
        else if (isTruthy(startsWithMatch)) keyScore = 0.9
        else if (isTruthy(wordBoundaryMatch)) keyScore = 0.7
        else keyScore = 0.5
...
export function highlightMatches(text: string, query: string, caseSensitive = false): string {
  ...
  return `${String(before ?? '')}<mark class="bg-yellow-200 dark:bg-yellow-800">${String(match ?? '')}</mark>${String(after ?? '')}`
}
