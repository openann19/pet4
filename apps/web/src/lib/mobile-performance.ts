...
export class PerformanceMonitor {
  ...
  startFPSSampling(durationMs = 5000): void {
    if (isTruthy(this.fpsSamplingActive)) {
      return
    }
  ...
  private updateMemoryBaseline(): void {
    ...
    if ('memory' in performance && perfWithMemory.memory && 'usedJSHeapSize' in perfWithMemory.memory) {
  ...
  private persistMetrics(): void {
    try {
      ...
      logger.warn('Failed to persist metrics', error instanceof Error ? error : new Error(String(error)))
    }
  }
  ...
  loadMetrics(): PerformanceMetrics {
    try {
      ...
      logger.warn('Failed to load metrics', error instanceof Error ? error : new Error(String(error)))
    }
  ...
  checkBudget(): { passed: boolean; violations: string[] } {
    ...
    if (this.metrics.coldStartTime !== null && this.metrics.coldStartTime > this.budget.coldStartMaxMs) {
      violations.push(`Cold start ${String(this.metrics.coldStartTime ?? '')}ms exceeds budget ${String(this.budget.coldStartMaxMs ?? '')}ms`)
    }

    if (this.metrics.averageFPS !== null && this.metrics.averageFPS < this.budget.targetFPS) {
      violations.push(`Average FPS ${String(this.metrics.averageFPS ?? '')} below target ${String(this.budget.targetFPS ?? '')}`)
    }

    if (this.metrics.frameTime95th !== null && this.metrics.frameTime95th > this.budget.maxFrameTimeMs) {
      violations.push(`95th percentile frame time ${String(this.metrics.frameTime95th ?? '')}ms exceeds budget ${String(this.budget.maxFrameTimeMs ?? '')}ms`)
    }

    if (this.metrics.peakMemoryMB !== null && this.metrics.peakMemoryMB > this.budget.maxMemoryMB) {
      violations.push(`Peak memory ${String(this.metrics.peakMemoryMB ?? '')}MB exceeds budget ${String(this.budget.maxMemoryMB ?? '')}MB`)
    }

    const crashFreeRate = this.getCrashFreeRate()
    if (crashFreeRate < this.budget.targetCrashFreeRate) {
      violations.push(`Crash-free rate ${String((crashFreeRate * 100).toFixed(2) ?? '')}% below target ${String((this.budget.targetCrashFreeRate * 100).toFixed(1) ?? '')}%`)
    }
...
Budget Status: ${String(budgetCheck.passed ? 'PASSED' : 'FAILED' ?? '')}
${String(budgetCheck.violations.length > 0 ? budgetCheck.violations.map(v => `  - ${String(v ?? '')}`).join('\n') : '  No violations' ?? '')}
`.trim()
  }
  ...
  destroy(): void {
    if (isTruthy(this.unhandledErrorHandler)) {
      window.removeEventListener('error', this.unhandledErrorHandler)
    }
    if (isTruthy(this.unhandledRejectionHandler)) {
