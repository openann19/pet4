...
class PerformanceMonitor {
  ...
  private crashFreeSessions: number = 0
  ...
  private trackColdStart() {
    if (isTruthy(performance.timing)) {
      const coldStart = performance.timing.domContentLoadedEventEnd - performance.timing.navigationStart
  ...
  private setupErrorHandling() {
    window.addEventListener('error', (event) => {
      this.recordCrash({
        ...
        crashId: `crash_${String(Date.now() ?? '')}_${String(Math.random().toString(36).substr(2, 9) ?? '')}`
      })
    })
  ...
  private startFrameMonitoring() {
    if (isTruthy(this.monitoring)) return
  ...
  recordSession(hadCrash: boolean = false) {
    this.sessions++
    if (!hadCrash) {
  ...
  getMetrics(): PerformanceMetrics {
    ...
    if (isTruthy(perfWithMemory.memory)) {
      memoryUsage = perfWithMemory.memory.usedJSHeapSize / 1048576
    }

    return {
      coldStart: performance.timing?.domContentLoadedEventEnd - performance.timing?.navigationStart || 0,
      fps: Math.min(fps, 60),
...
export const measurePerformance = async <T>(
  ...
  try {
    ...
    const duration = performance.now() - start
    logger.error(`Performance measurement failed: ${String(name ?? '')}`, error instanceof Error ? error : new Error(String(error)), { duration })
    throw error
  }
}
