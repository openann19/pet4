...
export class GDPRService {
  ...
  async exportUserData(userId: string): Promise<UserDataExport> {
    try {
      ...
      throw new Error(`Failed to export user data: ${String(errorMessage ?? '')}`)
    }
  }
  ...
  async deleteUserData(userId: string): Promise<DataDeletionResult> {
    try {
      ...
      throw new Error(`Failed to delete user data: ${String(errorMessage ?? '')}`)
    }
  }
  ...
  async requestDataExport(userId: string): Promise<UserDataExport> {
    try {
      const response = await api.get<UserDataExport>(`/api/gdpr/export/${String(userId ?? '')}`)
  ...
  async requestDataDeletion(userId: string): Promise<DataDeletionResult> {
    try {
      const response = await api.post<DataDeletionResult>(`/api/gdpr/delete/${String(userId ?? '')}`, {})
