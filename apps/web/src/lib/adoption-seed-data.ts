...
export async function generateAdoptionProfiles(count: number = 12): Promise<AdoptionProfile[]> {
  const prompt = buildLLMPrompt`Generate exactly ${String(count ?? '')} diverse pet adoption profiles. 
  ...
  const response = await llmService.llm(prompt, 'gpt-4o', true)
    ...
    const profiles: AdoptionProfile[] = profilesData.map((data: LLMProfileData, index: number) => ({
      ...
      photos: [
        `https://picsum.photos/seed/adopt-${String(Date.now() ?? '')}-${String(index ?? '')}/800/600`,
        `https://picsum.photos/seed/adopt-${String(Date.now() ?? '')}-${String(index ?? '')}-2/800/600`,
        `https://picsum.photos/seed/adopt-${String(Date.now() ?? '')}-${String(index ?? '')}-3/800/600`
      ],
