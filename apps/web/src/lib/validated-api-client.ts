...
export class ValidatedAPIClient {
  ...
  private async request<T>(
    ...
    if (isTruthy(this.accessToken)) {
      (headers as Record<string, string>)['Authorization'] = `Bearer ${String(this.accessToken ?? '')}`
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (isTruthy(errorResult.success)) {
...
export function handleAPIError(error: unknown): string {
  ...
  if (error instanceof ValidationError) {
    ...
    if (isTruthy(firstError)) {
      return `Validation error: ${String(firstError.path.join('.') ?? '')} - ${String(firstError.message ?? '')}`
    }
