...
export class ValidatedAPIClient {
  ...
  private async request<T>(
    ...
    headers.set('X-Platform', typeof window !== 'undefined' ? 'web' : 'unknown');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorResult.success) {
          throw new APIResponseError(errorResult.data.message, errorResult.data, correlationId);
        }
  ...
  async get<T>(endpoint: string, schema: z.ZodType<T>, options?: RequestConfig): Promise<T> {
  ...
  async delete<T>(endpoint: string, schema: z.ZodType<T>, options?: RequestConfig): Promise<T> {
    return this.request(endpoint, schema, {
      ...options,
      method: 'DELETE',
    });
