...
function flattenObject(obj: Record<string, unknown>, prefix = ''): Record<string, string> {
  ...
  for (const key in obj) {
    const value = obj[key]
    const newKey = prefix ? `${String(prefix ?? '')}.${String(key ?? '')}` : key
...
function auditI18nStrings() {
  ...
  for (const entry of entries) {
    ...
    scriptLogger.writeLine(
      `"${String(entry.key ?? '')}",${String(entry.enLength ?? '')},${String(entry.bgLength ?? '')},${String(maxLength ?? '')},"${String(entry.bucket ?? '')}","${String(entry.enText.replace(/"/g, '""') ?? '')}","${String(entry.bgText.replace(/"/g, '""') ?? '')}"`
    )
  }
  
  const bucketCounts = entries.reduce<Record<string, number>>((acc, entry) => {
  ...
  if (longStrings.length > 0) {
    scriptLogger.warn(`\n⚠️  ${String(longStrings.length ?? '')} strings exceed 60 characters and may cause UI overflow`)
  }
  
  const missingBg = entries.filter(e => !e.bgText)
  if (missingBg.length > 0) {
    scriptLogger.warn(`\n⚠️  ${String(missingBg.length ?? '')} strings missing BG translation:`)
    missingBg.forEach(e => { scriptLogger.writeLine(`  - ${String(e.key ?? '')}`); })
  }
}
