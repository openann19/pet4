...
const reactNativeTransformPlugin = () => {
  return {
    ...
    resolveId(id: string, importer?: string) {
      ...
      if (id === 'react-native') {
        if (importer && (importer.includes('packages/') || importer.includes('/motion/') || importer.includes('packages/motion'))) {
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: 'react-native-reanimated', replacement: path.resolve(projectRoot, './src/lib/reanimated-web-polyfill.ts') },
      { find: '@petspark/shared', replacement: path.resolve(projectRoot, '../../packages/shared/src') },
      { find: '@petspark/motion', replacement: path.resolve(projectRoot, '../../packages/motion/src') },
      { find: '@petspark/config', replacement: path.resolve(projectRoot, '../../packages/config/src') },
      { find: '@petspark/chat-core', replacement: path.resolve(projectRoot, '../../packages/chat-core/src') },
  ...
  optimizeDeps: {
    ...
    esbuildOptions: {
      resolveExtensions: ['.web.js', '.web.jsx', '.web.ts', '.web.tsx', '.jsx', '.js', '.tsx', '.ts', '.json'],
    },
  },
  ssr: {
