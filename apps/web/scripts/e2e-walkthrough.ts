...
export async function runWalkthrough(): Promise<WalkthroughResult> {
  ...
  const step = async (name: string, fn: () => Promise<void>): Promise<void> => {
    ...
    try {
      logger.info(`Step: ${String(name ?? '')}`)
      await fn()
      const duration = Date.now() - stepStart
      steps.push({ name, success: true, duration })
      logger.info(`✓ ${String(name ?? '')} completed in ${String(duration ?? '')}ms`)
    } catch (error) {
      const duration = Date.now() - stepStart
      const errorMessage = error instanceof Error ? error.message : String(error)
      steps.push({ name, success: false, duration, error: errorMessage })
      logger.error(`✗ ${String(name ?? '')} failed: ${String(errorMessage ?? '')}`)
      throw error
    }
  }
  ...
  try {
    ...
    await step('1. Health Check', async () => {
      ...
      if (health.status !== 'healthy') {
        throw new Error(`Health check failed: ${String(health.status ?? '')}`)
      }
    })

    await step('2. User Registration', async () => {
      const email = `demo-${String(generateULID() ?? '')}@example.com`
    ...
    logger.info(`Walkthrough completed successfully in ${String(totalDuration ?? '')}ms`)
    ...
    const totalDuration = Date.now() - startTime
    const errorMessage = error instanceof Error ? error.message : String(error)
    logger.error(`Walkthrough failed: ${String(errorMessage ?? '')}`, error instanceof Error ? error : new Error(errorMessage))
...
if (import.meta.url === `file://${String(process.argv[1] ?? '')}`) {
  runWalkthrough()
    ...
    .catch(error => {
      logger.error('Walkthrough execution failed', error instanceof Error ? error : new Error(String(error)))
      process.exit(1)
    })
}
