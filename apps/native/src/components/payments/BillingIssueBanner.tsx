...
export const BillingIssueBanner: React.FC<BillingIssueBannerProps> = ({
  ...
  useEffect(() => {
    const calculateTimeRemaining = () => {
      ...
      if (days > 0) {
        setTimeRemaining(`${String(days ?? '')} day${String(days > 1 ? 's' : '' ?? '')} left`);
      } else if (hours > 0) {
        setTimeRemaining(`${String(hours ?? '')} hour${String(hours > 1 ? 's' : '' ?? '')} left`);
      } else {
        const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
        setTimeRemaining(`${String(minutes ?? '')} minute${String(minutes > 1 ? 's' : '' ?? '')} left`);
      }
    };
