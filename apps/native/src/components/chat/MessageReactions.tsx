...
function normalizeReactions(
  ...
  if (firstReaction && ('userId' in firstReaction || 'userIds' in firstReaction)) {
    ...
    const grouped = messageReactions.reduce<Record<string, { emoji: string; users: string[]; reactions: MessageReaction[] }>>((acc, reaction) => {
      ...
      {normalizedReactions.length > 0 && (
        <View style={styles.reactionsDisplay}>
          {normalizedReactions.map((reaction) => {
            ...
            return (
              <ReactionButton
                key={`${String(messageId ?? '')}-${String(reaction.emoji ?? '')}`}
                emoji={reaction.emoji}
...
function ReactionButton({
  ...
  return (
    <Pressable
      ...
      accessibilityRole="button"
      accessibilityLabel={`${String(emoji ?? '')} reaction, ${String(count ?? '')} ${String(count === 1 ? 'person' : 'people' ?? '')}`}
