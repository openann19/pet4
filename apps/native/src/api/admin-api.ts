...
class MobileAdminApiImpl {
  ...
  async getSystemStats(): Promise<SystemStats> {
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/analytics`);
  ...
  async resetUserPassword(
    ...
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/users/${String(userId ?? '')}/reset-password`, {
        method: 'POST',
  ...
  async getUserDetails(userId: string): Promise<AdminUser> {
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/users/${String(userId ?? '')}`);
  ...
  async broadcastConfig(
    ...
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/config/broadcast`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          configType,
  ...
  async getAuditLogs(limit: number = 100): Promise<AdminAction[]> {
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/audit-logs?limit=${String(limit ?? '')}`);
  ...
  async createAuditLog(entry: Omit<AdminAction, 'id' | 'timestamp'>): Promise<void> {
    try {
      await fetch(`${String(this.baseUrl ?? '')}/audit-logs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...entry,
  ...
  async moderatePhoto(
    ...
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/moderation/photos/${String(taskId ?? '')}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      logger.error('Failed to moderate photo:', error);
      throw error;
    }
  }
  ...
  async getKYCQueue(): Promise<Array<{
    ...
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/kyc/queue`);
      if (!response.ok) throw new Error('Failed to fetch KYC queue');
      const data = await response.json();
      return data.pending || [];
    } catch (error) {
      logger.error('Failed to get KYC queue:', error);
      return [];
    }
  }
  ...
  async reviewKYC(
    ...
    try {
      const response = await fetch(`${String(this.baseUrl ?? '')}/kyc/sessions/${String(sessionId ?? '')}/review`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action, reason }),
      });
      ...
      logger.error('Failed to review KYC:', error);
      throw error;
    }
  }
}
