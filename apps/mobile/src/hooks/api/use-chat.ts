...
async function fetchMessages(chatRoomId: string, cursor?: string): Promise<{ items: Message[]; nextCursor?: string }> {
  const url = cursor
    ? `${String(API_BASE_URL ?? '')}/api/chat/${String(chatRoomId ?? '')}/messages?cursor=${String(cursor ?? '')}`
    : `${String(API_BASE_URL ?? '')}/api/chat/${String(chatRoomId ?? '')}/messages`
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch messages: ${String(response.statusText ?? '')}`)
  }
...
async function sendMessage(chatRoomId: string, content: string): Promise<Message> {
  const response = await fetch(`${String(API_BASE_URL ?? '')}/api/chat/${String(chatRoomId ?? '')}/messages`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!response.ok) {
    throw new Error(`Failed to send message: ${String(response.statusText ?? '')}`)
  }
...
async function markMessageAsRead(chatRoomId: string, messageId: string): Promise<{ success: boolean }> {
  const response = await fetch(`${String(API_BASE_URL ?? '')}/api/chat/${String(chatRoomId ?? '')}/messages/${String(messageId ?? '')}/read`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!response.ok) {
    throw new Error(`Failed to mark message as read: ${String(response.statusText ?? '')}`)
  }
