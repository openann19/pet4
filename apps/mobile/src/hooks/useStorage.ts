...
export function useStorage<T>(key: string, defaultValue: T): [T, (value: T | ((prev: T) => T)) => void, () => void] {
  ...
  useEffect(() => {
    ...
    const loadValue = async () => {
      try {
        ...
        logger.error(`Failed to load value for key ${String(key ?? '')}`, error instanceof Error ? error : new Error(String(error)))
        if (!cancelled) {
  ...
  const setValue = useCallback(async (newValue: T | ((prev: T) => T)) => {
    try {
      ...
      logger.error(`Failed to set value for key ${String(key ?? '')}`, error instanceof Error ? error : new Error(String(error)))
      ...
      try {
        ...
        logger.error(`Failed to revert value for key ${String(key ?? '')}`, revertError instanceof Error ? revertError : new Error(String(revertError)))
      }
  ...
  const deleteValue = useCallback(async () => {
    try {
      ...
      logger.error(`Failed to delete value for key ${String(key ?? '')}`, error instanceof Error ? error : new Error(String(error)))
    }
  }, [key])
