...
export default function AdvancedChatWindow({
  ...
  const [messages, setMessages] = useKV<ChatMessage[]>(`chat-messages-${String(room.id ?? '')}`, [])
      ...
      <ScrollView ref={scrollRef} style={styles.messages} contentContainerStyle={{ padding: 16 }}>
        {messages.map((msg) => (
          <MessageBubble
            ...
            onReact={(msgId) => {
              toast.info(`React to message ${String(msgId ?? '')}`)
              haptics.trigger('medium')
            }}
            onLongPress={(msgId) => {
              toast.warning(`Long press on message ${String(msgId ?? '')}`)
