...
import { Animated, useSharedValue, useAnimatedStyle, withSpring, withTiming, withRepeat } from '@petspark/motion'
...
export function PremiumProgress({
  ...
  return (
    <View style={[styles.container, style]} testID={testID} accessibilityLabel={ariaLabel}>
      ...
      <View style={[styles.track, sizes[size]]}>
        <Animated.View
          style={[
            styles.progress,
            progressStyle,
        ...
        >
          {variant === 'striped' && (
            <Animated.View style={[styles.shimmer, shimmerStyle, { height: sizes[size].height }]} />
          )}
        </Animated.View>
      </View>
    </View>
