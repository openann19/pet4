...
export function SmartSearch<T extends Record<string, unknown>>({
  ...
  const renderItem = ({ item: result }: { item: { item: T; score: number; matchedKey: string } }) => {
    if (isTruthy(renderResult)) {
      return (
        <Pressable
          onPress={() => { handleSelect(result.item, query); }}
  ...
  const renderHistoryItem = ({ item: historyQuery }: { item: string }) => (
    <Pressable
  ...
  const renderTrendingItem = ({ item: trending }: { item: string }) => (
    <Pressable
