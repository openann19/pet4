      ...
      <PullableContainer onRefresh={handleRefresh}>
        ...
        >
          ...
          <FeatureCard title="Post moderation">
            <BodyLine text={`Pending posts can be edited: ${String(community.canEditPendingPost ? 'Yes' : 'No' ?? '')}`} />
            <BodyLine
          ...
          <FeatureCard title="Post transitions">
            {community.postTransitions.map((item) => (
              <StatusRow key={`post:${String(item.status ?? '')}`} label={item.status} allowed={item.allowed} />
            ))}
          </FeatureCard>

          <FeatureCard title="Comment transitions">
            {community.commentTransitions.map((item) => (
              <StatusRow key={`comment:${String(item.status ?? '')}`} label={item.status} allowed={item.allowed} />
            ))}
          </FeatureCard>
        </ScrollView>
      </PullableContainer>
