 ...
 */
export function analyzePersonalityCompatibility(
  pet1: Pet,
  pet2: Pet
): PersonalityAnalysis {
  ...
  const compatibility = traits1.length > 0 && traits2.length > 0
    ? (commonTraits.length / Math.max(traits1.length, traits2.length)) * 100
    : 50;

  const traitScores = [
    ...new Set([...traits1, ...traits2]),
  ].map((trait) => ({
    trait,
