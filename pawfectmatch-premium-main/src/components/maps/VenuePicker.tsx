...
import { MapPin, X, Search, NavigationArrow, Clock, Star } from '@phosphor-icons/react';
...
import { searchPlaces } from '@/lib/maps/mapbox-places';
...
export default function VenuePicker({
  ...
  const loadVenues = async (): Promise<void> => {
    ...
    try {
      ...
      const results = await searchPlaces(userLocation, query, selectedCategory || undefined);
      setVenues(results.slice(0, 20));
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error));
      toast.error(t.map?.errorLoadingMap || 'Error loading venues');
    } finally {
      setLoading(false);
    }
  };
      ...
      const venueIcon = L.divIcon({
        ...
        html: `
          <div class="venue-marker-container">
            <div class="venue-marker-icon">${VENUE_CATEGORIES.find(c => c.id === venue.category)?.icon || '📍'}</div>
            <div class="venue-marker-name">${venue.name}</div>
          </div>
        `,
        iconSize: [60, 40],
        iconAnchor: [30, 40],
