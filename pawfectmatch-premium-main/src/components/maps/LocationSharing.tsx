...
import { getCurrentLocation, snapToGrid, formatDistance } from '@/lib/maps/utils';
...
export function LocationBubble({
  ...
  onTap,
  className,
}: LocationBubbleProps): JSX.Element {
  const { t } = useApp();
          ...
          <div className="absolute bottom-2 left-2 right-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1.5 text-white text-xs font-medium">
                ...
                <span>{label || t.map?.location || 'Location'}</span>
              </div>
              {timestamp && (
...
export function LocationPicker({
  ...
  useEffect(() => {
    getCurrentLocation()
      ...
      .catch(() => {
        setUserLocation(currentLocation || { lat: 40.7128, lng: -74.006 });
