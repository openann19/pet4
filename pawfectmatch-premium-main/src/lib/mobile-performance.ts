...
export class PerformanceMonitor {
  ...
  checkBudget(): { passed: boolean; violations: string[] } {
    ...
    if (this.metrics.coldStartTime !== null && this.metrics.coldStartTime > this.budget.coldStartMaxMs) {
      violations.push(`Cold start ${this.metrics.coldStartTime}ms exceeds budget ${this.budget.coldStartMaxMs}ms`)
    }

    if (this.metrics.averageFPS !== null && this.metrics.averageFPS < this.budget.targetFPS) {
      violations.push(`Average FPS ${this.metrics.averageFPS} below target ${this.budget.targetFPS}`)
    }

    if (this.metrics.frameTime95th !== null && this.metrics.frameTime95th > this.budget.maxFrameTimeMs) {
      violations.push(`95th percentile frame time ${this.metrics.frameTime95th}ms exceeds budget ${this.budget.maxFrameTimeMs}ms`)
    }

    if (this.metrics.peakMemoryMB !== null && this.metrics.peakMemoryMB > this.budget.maxMemoryMB) {
      violations.push(`Peak memory ${this.metrics.peakMemoryMB}MB exceeds budget ${this.budget.maxMemoryMB}MB`)
    }

    const crashFreeRate = this.getCrashFreeRate()
    if (crashFreeRate < this.budget.targetCrashFreeRate) {
      violations.push(`Crash-free rate ${(crashFreeRate * 100).toFixed(2)}% below target ${(this.budget.targetCrashFreeRate * 100).toFixed(1)}%`)
    }
